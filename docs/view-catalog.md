...
| Navigation | ready for controlled shells and route focus restoration | `navigation_stack`, `router_stack`, `route_header`, `section_nav`, `sidebar`, `breadcrumb`, `split_view`, `master_detail`, `wizard`, `tab_view`, `@core.RouteFocusStore`, `AppRuntime::focus_key` | Showcase Navigation Shell; Showcase router/state tests | Deep-link integration, browser history, animated route transitions, and resizable split panes remain app/host follow-up work. Route focus restoration is preview-ready as an app/host call into the runtime after a route switch. |
...
| Data views | ready with app-owned data logic | `DataSortState`, `PaginationState`, `ColumnVisibilityState`, `SelectionState`, `data_filter_bar`, `data_filter`, `selection_toolbar`, `table`, `table_column`, `column_visibility_panel`, `selectable_list`, `pagination`, `detail_panel`, `tree_view`, `description_list`, `avatar`, `avatar_group`, `virtual_list`, `sectioned_list`, `scroll_to_index`, `lazy_list`, `lazy_grid` | Showcase Data and Layout; upgraded Data Table example | Filtering rules, sorting rules, request lifecycle, column resizing/reordering, and row virtualization policy stay in app packages today; reusable helpers cover predictable sort, page, visibility, and selection state. |
...
| `DataSortState` / `PaginationState` / `ColumnVisibilityState` / `SelectionState` | `views/style_api.mbt` | N/A | N/A | `views/views_test.mbt` | Showcase Data, Data Table | Pure app-owned helpers for common data workflows: sort toggles, page clamping and slicing bounds, visible/locked columns, and selected ids. They intentionally do not own filtering predicates, requests, timers, or bulk effects. |
...
| `table` / `table_column` | `views/data_display.mbt` | Surface/text/border/ButtonStyle | Grid with selectable row items and optional sortable header buttons | `views/views_test.mbt` | Showcase Data, Data Table | Controlled text table with optional custom cell view, selected row, row-select messages, sortable header state, and empty state; `DataSortState` can drive header state while sorting rules, filtering, and resizing stay app-owned. |
| `column_visibility_panel` | `views/data_display.mbt` | Checkbox/text/surface | Group with selected and disabled column items | `views/views_test.mbt` | Showcase Data, Data Table | Controlled column visibility chooser for table workflows. `ColumnVisibilityState` can store visible and locked ids while column resizing and reordering remain follow-up work. |
...
| `dialog_host` | `views/navigation.mbt` | SurfaceStyle | Dialog when presented | `views/views_test.mbt`, Showcase tests | Showcase | Stack wrapper for modal content. Presented dialogs apply view-level focus trap by default; pass `trap_focus=false` only for intentionally non-modal preview surfaces. |
