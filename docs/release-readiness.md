...
| Gradient | Visual styling and sparkline/card brushes. | Showcase app tests assert `FillRoundedRectBrush` with `LinearGradient` and `RadialGradient`; renderer tests cover Skia radial brush pixels and WGPU/Web deterministic radial rounded fill/stroke fallback. | Covered by visible demo plus command-level app test. |
