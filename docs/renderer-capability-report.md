...
| Text | ready | ready | ready | Skia `Font` measurement, font-metric baseline/height, measured caret positions, and best-available glyph-run rendering resolve `FontSpec` family, weight, and style through Skia `FontMgr`/`Font`, with optional SkShaper shaped-run rendering when linked and real native renderer smoke evidence; broader shaping is tracked separately. |
