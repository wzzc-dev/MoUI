...
#define MOUI_EMBEDDING_API_ENABLE_SCROLL 0
#endif
...
[[noreturn]] void fail(const char *expression, int line) {
  std::fprintf(
      stderr,
      "embedding_api_v1_test.cpp:%d: check failed: %s\n",
      line,
      expression);
  std::exit(1);
}
...
void release_buffer(moui_embedding_utf8_buffer_v1 *buffer) {
  CHECK(buffer->status == MOUI_EMBEDDING_API_STATUS_OK_V1);
...
void test_consumer_negotiation(const moui_embedding_api_v1 *api) {
  CHECK(moui_embedding_api_v1_is_compatible(api) != 0);
  CHECK(api->struct_size == MOUI_EMBEDDING_API_V1_REQUIRED_SIZE);

  moui_embedding_api_v1 candidate = *api;
  candidate.abi_version = MOUI_EMBEDDING_API_ABI_VERSION_V1 + 1;
  CHECK(moui_embedding_api_v1_is_compatible(&candidate) == 0);

  candidate = *api;
  candidate.struct_size = MOUI_EMBEDDING_API_V1_REQUIRED_SIZE - 1;
  CHECK(moui_embedding_api_v1_is_compatible(&candidate) == 0);

  candidate = *api;
  candidate.struct_size = MOUI_EMBEDDING_API_V1_REQUIRED_SIZE + 64;
  CHECK(moui_embedding_api_v1_is_compatible(&candidate) != 0);
  ...
  candidate = *api;
#if MOUI_EMBEDDING_API_ENABLE_SCROLL
  candidate.capabilities &= ~MOUI_EMBEDDING_API_CAPABILITY_SCROLL_V1;
  CHECK(moui_embedding_api_v1_is_compatible(&candidate) == 0);
...
void test_table_and_initialization(const moui_embedding_api_v1 *api) {
  CHECK(api != nullptr);
  CHECK(api == moui_embedding_get_api_v1());
  CHECK(api->abi_version == MOUI_EMBEDDING_API_ABI_VERSION_V1);
  CHECK(api->struct_size == sizeof(moui_embedding_api_v1));

  uint64_t expected_capabilities =
      MOUI_EMBEDDING_API_REQUIRED_CAPABILITIES_V1;
#if MOUI_EMBEDDING_API_ENABLE_SCROLL
  expected_capabilities |= MOUI_EMBEDDING_API_CAPABILITY_SCROLL_V1;
#endif
  ...
  const moui_embedding_utf8_view_v1 empty_text = {nullptr, 0};
  const moui_embedding_bytes_view_v1 empty_bytes = {nullptr, 0};
  ...
  CHECK(api->configure_renderer(empty_text) ==
        MOUI_EMBEDDING_API_ERROR_NOT_INITIALIZED_V1);
  const moui_embedding_utf8_buffer_v1 preinit = api->renderer_status_json();
  CHECK(preinit.status == MOUI_EMBEDDING_API_ERROR_NOT_INITIALIZED_V1);
  CHECK(preinit.data == nullptr);
  CHECK(preinit.length == 0);
  CHECK(preinit.release_context == nullptr);
  CHECK(preinit.release == nullptr);
  const moui_embedding_utf8_buffer_v1 preinit_updates =
  ...
  const moui_embedding_utf8_view_v1 app_view = {
  ...
  for (int32_t result : results) {
    CHECK(result == MOUI_EMBEDDING_API_RESULT_TRUE_V1);
  }
...
void test_lifecycle_surface_input_and_frame(
  ...
  CHECK(api->dispatch_pointer(
            ...
            20.5,
            30.75) == MOUI_EMBEDDING_API_ERROR_INVALID_ARGUMENT_V1);
  ...
  CHECK(api->detach_surface() == MOUI_EMBEDDING_API_RESULT_TRUE_V1);
...
void test_utf8_and_ownership(const moui_embedding_api_v1 *api) {
  const moui_embedding_utf8_view_v1 payload = {
  ...
  if (sizeof(size_t) > sizeof(int32_t)) {
    CHECK(api->configure_renderer(
          ...
          MOUI_EMBEDDING_API_ERROR_INPUT_TOO_LARGE_V1);
  }

  moui_embedding_utf8_buffer_v1 status = api->renderer_status_json();
  CHECK(status.status == MOUI_EMBEDDING_API_STATUS_OK_V1);
  ...
  moui_embedding_utf8_buffer_v1 updates =
      api->take_host_update_envelope_json();
  CHECK(updates.status == MOUI_EMBEDDING_API_STATUS_OK_V1);
  ...
  CHECK(status.status == MOUI_EMBEDDING_API_ERROR_INVALID_UTF16_V1);
...
void test_host_services(const moui_embedding_api_v1 *api) {
  const moui_embedding_utf8_view_v1 payload = {
      kUtf8Payload, std::size(kUtf8Payload)};
...
void test_destroy_is_terminal(const moui_embedding_api_v1 *api) {
  ...
  CHECK(std::count(
            destroy_results.begin(),
            destroy_results.end(),
            MOUI_EMBEDDING_API_ERROR_APPLICATION_DESTROYED_V1) == 15);
  CHECK(g_destroy_calls == 1);
  CHECK(g_mock_application_state == -1);

  const uint8_t app_argument[] = {'a', 'p', 'p'};
  const moui_embedding_utf8_view_v1 text = {
      app_argument, std::size(app_argument)};
  const moui_embedding_bytes_view_v1 bytes = {nullptr, 0};
  CHECK(api->initialize(text) ==
        MOUI_EMBEDDING_API_ERROR_APPLICATION_DESTROYED_V1);
  CHECK(api->destroy_application() ==
        MOUI_EMBEDDING_API_ERROR_APPLICATION_DESTROYED_V1);
  CHECK(api->detach_surface() ==
        MOUI_EMBEDDING_API_ERROR_APPLICATION_DESTROYED_V1);
  ...
  const moui_embedding_utf8_buffer_v1 renderer_status =
  ...
  const moui_embedding_utf8_buffer_v1 host_updates =
      api->take_host_update_envelope_json();
...
int main() {
  const moui_embedding_api_v1 *api = moui_embedding_get_api_v1();
  ...
  std::puts("shell runtime ABI v1 tests passed");
  return 0;
}
