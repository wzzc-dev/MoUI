...
usage() {
  cat <<'USAGE'
Usage: moui_shell/scripts/build-harmonyos-hap.sh --app <id> [options]

Build a MoUI HarmonyOS HAP through the package-owned managed shell.

Options:
  --app <id>                    Shell app id.
  --harmonyos-project <dir>     Versioned ejected project.
  ...
  --ejected-shell               Build a versioned ejected shell.
...
if [ "$ejected_shell" -eq 0 ] && [ -n "$harmonyos_project" ]; then
  echo "--harmonyos-project requires --ejected-shell" >&2
  exit 2
fi
...
if [ "$ejected_shell" -eq 1 ]; then
  ...
  if [ -z "$project_root" ]; then project_root="$(dirname "$harmonyos_project")"; elif [ "${project_root#/}" = "$project_root" ]; then project_root="$workspace_root/$project_root"; fi
  node "$shell_root/scripts/validate-ejected-lock.mjs" \
    --lock "$shell_lock" --platform harmonyos --moui-root "$moui_root" \
...
if [ "$ejected_shell" -eq 1 ]; then
  ...
  node "$shell_root/harmonyos/runner/resolve-shell.mjs" "${resolver_args[@]}"
fi
...
if [ "$fallback_skia" -eq 0 ]; then
  ...
  (
    ...
    env \
      ...
      MOUI_EMBEDDING_NATIVE_CONFIG="$cmake_config" \
...
# Managed and ejected builds may reuse one build directory while pointing CMake
# at different shell source trees. Never carry a source-bound CMake cache across
# those modes.
