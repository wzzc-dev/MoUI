...
This directory contains validation and native Skia smoke helpers for `moui_skia`.
...
`verify-native-fallback-parity.*` keeps each `native/*_native.mbt` file paired
...
`wasm`/`wasm-gc`/`js`.
`verify-example-link-config.*` keeps checked-in example packages on build-script
link variables instead of provider-cache paths, so examples inherit the same
portable Skia link configuration as the native package.
...
the saved native executable log for `moui_skia native smoke test passed`, and
...
```bash
export MOUI_SKIA_SKIA_INCLUDE=/path/to/skia
export MOUI_SKIA_SKIA_LIB_DIR=/path/to/skia/out/Static
export MOUI_SKIA_SKIA_LIB=skia
export MOUI_SKIA_EXTRA_LINK_FLAGS="-lfontconfig -lfreetype -lharfbuzz -lpthread -ldl -lm"
...
selection is `--skia-rev`, then `MOUI_SKIA_SKIA_REV`, then the first non-comment
line of `skia-revision.txt`, then `main`. Source-build helpers also read
`MOUI_SKIA_EXTRA_GN_ARGS` when `--extra-gn-args` is omitted.
...
`logs/linux-native-smoke-output.log` for `moui_skia native smoke test passed` and
...
Pass `--enable-asan` or set `MOUI_SKIA_ENABLE_ASAN=1` on Linux/macOS smoke
...
`MOUI_SKIA_SKIA_REV`, then `skia-revision.txt`, matching Linux's revision
...
checks the final `moui_skia native smoke test passed` marker; and verifies that
...
Skia layout. The MSVC helper also accepts `MOUI_SKIA_SKIA_ROOT`,
`MOUI_SKIA_SKIA_INCLUDE`, `MOUI_SKIA_SKIA_ZIP`, `MOUI_SKIA_SKIA_LIB_DIR`,
`VCVARSALL`, `MOUI_SKIA_EXTRA_CC_FLAGS`, and `MOUI_SKIA_EXTRA_LINK_FLAGS` as
...
`moui_skia native smoke test passed` marker; and verifies that `native/moon.pkg`
...
Linux: `MOUI_SKIA_SKIA_INCLUDE`, `MOUI_SKIA_SKIA_LIB_DIR`, `MOUI_SKIA_SKIA_LIB`,
`MOUI_SKIA_EXTRA_CC_FLAGS`, and `MOUI_SKIA_EXTRA_LINK_FLAGS`. Explicit PowerShell
