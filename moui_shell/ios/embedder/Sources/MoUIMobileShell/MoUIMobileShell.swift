...
public enum MOUIShellStatusBarMode: String {
  case auto
  case visible
  case hidden
}

public enum MOUIShellOrientation: String {
  case any
  case portrait
  case landscape
...
public struct MOUIShellConfiguration {
  public let appArgument: String
  public let renderer: String
  public let fullscreen: Bool
  public let statusBar: MOUIShellStatusBarMode
  public let orientation: MOUIShellOrientation
  public let plugins: [MOUIShellPlugin.Type]

  public init(
    appArgument: String,
    renderer: String = "auto",
    fullscreen: Bool = false,
    statusBar: MOUIShellStatusBarMode = .auto,
    orientation: MOUIShellOrientation = .any,
    plugins: [MOUIShellPlugin.Type] = []
  ) {
...
private final class MOUIShellSystemPolicy {
  static let shared = MOUIShellSystemPolicy()
  var orientation: MOUIShellOrientation = .any

  private init() {}
}

public final class MOUIShellApplicationDelegate: NSObject, UIApplicationDelegate {
  public func application(
    _ application: UIApplication,
    supportedInterfaceOrientationsFor window: UIWindow?
  ) -> UIInterfaceOrientationMask {
    MOUIShellSystemPolicy.shared.orientation.mask
  }

  public func applicationWillTerminate(_ application: UIApplication) {
    MOUIShellPluginCapabilities.shared.resetSession()
    _ = MOUIShellRuntimeBridge.shared.destroyApplication()
  }
}

private final class MOUIShellSceneLease {
  static let shared = MOUIShellSceneLease()
...
final class MOUIShellSceneIdentity: ObservableObject {
  let value = UUID().uuidString
}

public struct MOUIShellRootView: View {
  private let configuration: MOUIShellConfiguration
  @Environment(\.scenePhase) private var scenePhase
  @StateObject private var sceneIdentity = MOUIShellSceneIdentity()

  public init(configuration: MOUIShellConfiguration) {
    self.configuration = configuration
  }

  @ViewBuilder
  public var body: some View {
    let surface = MOUIShellSurfaceRepresentable(
...
public struct MOUIShellSurfaceRepresentable: UIViewRepresentable {
  let configuration: MOUIShellConfiguration
  ...
  public final class Coordinator: NSObject {
    ...
    private let pluginCapabilities = MOUIShellPluginCapabilities.shared
    private weak var surface: MOUIMetalSurfaceView?
    private var hostAdapter: MOUIEmbedderHostAdapter?
    ...
    init(configuration: MOUIShellConfiguration, sceneIdentifier: String) {
      self.configuration = configuration
    ...
    func makeView() -> UIView {
      ...
      let sceneStatus = MOUIShellSceneLease.shared.acquire(identifier: sceneIdentifier)
      guard sceneStatus > 0 else {
        NSLog("moui-shell ios rejected additional scene status=%d", sceneStatus)
        return root
      }
      guard bridge.compatible else {
        NSLog("moui-shell ios runtime ABI v1 is incompatible")
        return root
      }
      ...
      guard started > 0 else {
        NSLog("moui-shell ios runtime start failed status=%d", started)
        return root
      }
      MOUIShellSystemPolicy.shared.orientation = configuration.orientation
      MOUIShellPluginRegistry.shared.install(
        configuration.plugins,
        capabilities: pluginCapabilities
      )
      hostAdapter = MOUIEmbedderHostAdapter(
        surface: surface,
        overlay: overlay,
    ...
    func dismantle() {
      detach()
      MOUIShellSceneLease.shared.release(identifier: sceneIdentifier)
      active = false
      surface = nil
      hostAdapter = nil
    }
