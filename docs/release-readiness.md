...
| --- | --- | --- |
| Daily baseline | `sh scripts/dev-check.sh --platform-examples-test` passed on Darwin on 2026-06-02 after the Skia descriptor integration, capture-manifest validator self-test, Web runtime handoff validator, local-dependency window evidence-surface guard, and macOS backend/provider checks. Real Skia smoke remains opt-in and was skipped because local real-Skia link flags were not requested. | current for Darwin host |
