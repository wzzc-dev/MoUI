...
| **L2 — 运行时行为** | 真实 Skia/WebGPU 下的像素级渲染验证 | 每次 PR (`moui-skia-real-skia-pr-smoke.yml`) | 匹配宿主 |
...
| 证据类型 | 状态 | 核验方法 | 最新通过记录 |
|----------|------|----------|-------------|
| L0: `moon check` | ✅ 通过 | `ci.yml → conformance` → `dev-check.sh` 含 `moon check` | 每次 PR |
| L0: `moon fmt --check` | ✅ 通过 | `ci.yml → conformance` → `moon fmt --check` | 每次 PR |
| L0: `moon info` | ✅ 无漂移 | `ci.yml → api-surface` → `moon info -p <pkg>` + `git diff --exit-code` | 每次 PR |
| L1: `moon test` 包测试 | ✅ 通过 | `ci.yml → conformance` → `moon test moui/{core,views,render,backend/host,...}` | 每次 PR |
| L1: 文本一致性测试 | ✅ 通过 | `ci.yml → conformance` → `conformance-check.sh --text --text-diagnostic` | 每次 PR |
| L2: 真实 Skia 渲染器 | ✅ 全 17 特性通过 | `moui-skia-real-skia-pr-smoke.yml → macos-real-skia` | 每次 PR |
| L2: 文本/表情符号 (SkParagraph) | ✅ 通过 | `moui-skia-real-skia-pr-smoke.yml → macos-real-skia --run-text-emoji-smoke` | 每次 PR |
| L2: 异步图像 | ✅ 通过 | `moui-skia-real-skia-pr-smoke.yml → macos-real-skia --run-renderer-smoke` | 每次 PR |
| L3: macOS 平台运行时证据 | ✅ 通过 | `MoUI CI macOS platform evidence dispatch` → GitHub Actions Run [27217345886](https://github.com/wzzc-dev/MoUI/actions/runs/27217345886) | 2026-06-17 |
...
| 证据类型 | 状态 | 核验方法 | 最新通过记录 |
|----------|------|----------|-------------|
| L0: `moon check` | ✅ 通过 | `ci.yml → windows-native` → `moon check`（跨平台，不在 Win CI 中单独跑但受 `dev-check.sh` 保护） | 每次 PR |
...
| L2: 真实 Skia 渲染器（Windows） | ✅ 通过 | `moui-skia-real-skia-pr-smoke.yml → windows-real-skia` | 每次 PR |
| L2: Windows 文本/表情符号 | ✅ 通过 | `moui-skia-real-skia-pr-smoke.yml → windows-real-skia --run-text-emoji-smoke` | 每次 PR |
| L2: Windows 异步图像 | ✅ 通过 | `moui-skia-real-skia-pr-smoke.yml → windows-real-skia --run-renderer-smoke` | 每次 PR |
...
**依赖条件**: Windows MSVC 工具链 + vcpkg zlib + 真实 Skia provider。CI 工作流 `moui-skia-windows-real-skia-smoke.yml` 已具备完整环境，可作为 `workflow_dispatch` 手动触发。

#### Windows 证据参考

| 可引用项 | 内容 |
|----------|------|
| CI 工作流 | `moui-skia-windows-real-skia-smoke.yml` — 支持 MSVC/MinGW 两种工具链 |
...
| 证据类型 | 状态 | 核验方法 | 最新通过记录 |
|----------|------|----------|-------------|
| L0: `moon check` | ✅ 通过 | `ci.yml → linux-platform` → `dev-check.sh --platform-examples-test` 含 `moon check` | 每次 PR |
| L0: `moon info` | ✅ 无漂移 | `ci.yml → api-surface` | 每次 PR |
| L1: Linux 后端测试 | ✅ 通过 | `ci.yml → linux-platform` → `dev-check.sh --platform-examples-test` | 每次 PR |
| L1: Linux 包测试 | ✅ 通过 | `ci.yml → linux-platform` — core/views/render/backend/host native 测试 | 每次 PR |
| L2: 真实 Skia 渲染器（Linux） | ✅ **部分通过** | `moui-skia-real-skia-pr-smoke.yml → linux-real-skia` — 17 渲染特性中 **14 项通过, 3 项因文本布局 pending** | 每次 PR |
...
| 证据类型 | 状态 | 核验方法 | 最新通过记录 |
|----------|------|----------|-------------|
| L0: 构建 | ✅ 通过 | `ci.yml → conformance` → Showcase/Markdown Editor Web wasm-gc 构建 | 每次 PR |
| L1: Web 后端测试 | ✅ 通过 | `ci.yml → conformance` → `moon test moui/backend/web --target wasm-gc` | 每次 PR |
| L1: WebGPU 适配器测试 | ✅ 通过 | `ci.yml → conformance` → `moon test moui/render/webgpu_adapter --target wasm-gc` | 每次 PR |
...
**核验入口**: `moui-skia-real-skia-pr-smoke.yml` 每次 PR 运行，所有三平台 job 同时通过。
...
**原因**: Linux 的 SkParagraph 和 emoji font fallback 路径与 macOS/Windows 的字体基础设施不同（fontconfig + FreeType vs CoreText vs DirectWrite），需额外调试。文本布局证据通过 `moui-skia-real-skia-pr-smoke.yml` PR job 自动跟踪进展，每次 PR 更新状态。
