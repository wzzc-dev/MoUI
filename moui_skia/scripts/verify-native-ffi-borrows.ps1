...
$toolPackage = "tools/moui_skia/verify_native_ffi_borrows"
...
if (!(Test-Path -LiteralPath $toolDir -PathType Container)) {
  throw "MoonBit native FFI borrow tool is missing: $toolDir"
}
...
try {
  ...
  $toolExe = Join-Path $workspaceRoot "_build/native/debug/build/wzzc-dev/moui_tools/moui_skia/verify_native_ffi_borrows/verify_native_ffi_borrows.exe"
  & $toolExe @toolArgs
