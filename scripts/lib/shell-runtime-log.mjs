...
export const mobileResizeDimensions = logs => {
  ...
  for (const match of logs.matchAll(
    /moui-shell (?:lifecycle attach|resize)[^\r\n]*?width=(\d+)[^\r\n]*?height=(\d+)/g,
  )) {
...
const hasAcceptedClipboardCompletion = (logs, operation) =>
  logs.split(/\r?\n/g).some(line =>
    line.includes(`moui-shell service clipboard complete operation=${operation}`)
...
export const parseMobileTestProbeSnapshot = logs => {
  ...
  let latest = null;
  for (const match of logs.matchAll(
    /moui-shell test-probe snapshot=(\{[^\r\n]*\})/g,
  )) {
    try {
...
///|
/// Parse the latest `moui-shell service probe plan textField=x,y action=x,y`
...
///|
/// Parse the last `moui-shell renderer configure ... status={...}` JSON from
/// runtime logs into the shell-runtime-smoke `renderer` block. Returns null
/// when no configure status is present.
export const parseShellRendererStatus = logs => {
  if (typeof logs !== "string" || !logs.includes("moui-shell renderer configure")) {
    return null;
  }
  let last = null;
  for (const match of logs.matchAll(
    /moui-shell renderer configure[^\r\n]*?status=(\{[^\r\n]*\})/g,
  )) {
    try {
...
///|
/// Fallback renderer block from shell-build.json when runtime logs lack a
...
///|
/// Pending seven-gate skeleton required by the shell runtime validator when
