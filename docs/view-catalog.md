...
| `column_visibility_panel` | `views/data_display.mbt` | Checkbox/text/surface | Group with selected and disabled column items | `views/views_test.mbt` | Showcase Data, Data Table | Controlled column visibility chooser for table workflows. `ColumnVisibilityState` stores visible and locked ids; pair it with `ColumnWidthState` and `ColumnOrderState` when a table workflow also needs app-owned resizing and reordering. |
