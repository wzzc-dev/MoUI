...
MoUI CI Web/renderer-proof evidence refreshed on 2026-06-06, and macOS-only
...
refreshed on 2026-06-08; macOS now has GitHub Actions renderer-proof provenance,
but cross-platform native paragraph/bidi readiness still requires matching
Windows and Linux SkParagraph renderer-proof manifests.
...
| Native Skia paragraph/bidi proof | `moui_skia` exposes `skia_paragraph_available()` plus the native `Paragraph` wrapper for UTF-8 layout, line metrics, selection boxes, and hit testing; `render/skia` uses it when `MOUI_SKIA_ENABLE_SKPARAGRAPH=1` is configured and keeps fallback readiness flags false otherwise. Renderer-proof validation now requires `engine=skparagraph native_paragraph_ready=true line-metrics later-line-pixels` for `paragraphWrapping`, `engine=skparagraph bidi_visual_order_ready=true visual-order` for `bidiLayout`, and `engine=skparagraph selection-rects line-range rect-geometry hit-test` for `selectionRects`. GitHub Actions run `27217345886` produced a non-skipped successful macOS Skia renderer-proof manifest with `github-actions` provenance and `status=passed`; the macOS platform runtime evidence artifact also contains a passed `skia-native-macos.json`. | macOS passed; Windows/Linux still pending for cross-platform native SkParagraph/bidi readiness |
...
This refresh records the current `main` MoUI CI and website evidence gathered
on 2026-06-06 UTC for head SHA
`5ef72f16e2d9d8e0c7ade5d5d26459ca1e13dcb7`, plus the macOS-only platform
runtime evidence gathered on 2026-06-09 UTC for head SHA
`5bb2d810cd7e4cc63602caa431d5851827bd69d9`. The binding-level companion
