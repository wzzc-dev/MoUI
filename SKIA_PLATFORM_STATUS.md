...
- The native smoke log passed `scripts/verify-native-smoke-log.*`, including
  ...
  text-run dependency planning, byte-range text-run resource keys, UTF-8 text
  measurement, glyph count, glyph ID mapping, glyph advances, glyph positions,
  glyph bounds, text bounds measurement, font resource planning, font metrics,
  font family enumeration, native typeface family metadata, FontMgr character
  fallback, font fallback resource planning, and shader/filter resource
  planning.
