...
usage() {
  cat <<'USAGE'
Usage: scripts/build-platform-showcase-android-apk.sh [options]

Build the experimental Platform Showcase Android debug APK.
...
Options:
  ...
  --build-dir <dir>   Working directory, default artifacts/android/platform_showcase.
  --output <apk>      APK output, default artifacts/android/platform_showcase/app-debug.apk.
...
api_level="23"
compile_sdk="35"
build_dir="$repo_root/artifacts/android/platform_showcase"
output_apk=""
fallback_skia=0
prepare_only=0
...
log() {
  printf '[moui-platform-showcase-android] %s\n' "$*"
}
...
log "Generating MoonBit native C for examples/platform_showcase/android"
(
  cd "$repo_root"
  MOUI_SKIA_DISABLE_PREBUILD_SKIA=1 \
  MOUI_PDFIUM_DISABLE_PREBUILD_PDFIUM=1 \
    moon build examples/platform_showcase/android --target native --target-dir "$moon_target_dir"
)

moonbit_c="$moon_target_dir/native/debug/build/examples/platform_showcase/android/android.c"
if [ ! -f "$moonbit_c" ]; then
...
cmake_args=(
  -S "$repo_root/examples/platform_showcase/android_app/app/src/main/cpp"
  -B "$cmake_build_dir"
  ...
  "-DMOUI_PLATFORM_SHOWCASE_ANDROID_MOONBIT_C=$moonbit_c"
...
native_lib="$(find "$cmake_build_dir" -name libplatform_showcase_android.so -type f | head -n 1)"
if [ -z "$native_lib" ]; then
  echo "native library was not produced under $cmake_build_dir" >&2
  exit 1
fi

app_src="$repo_root/examples/platform_showcase/android_app/app/src/main"
...
sed 's#<manifest xmlns:android=#<manifest package="dev.wzzc.moui.platformshowcase" xmlns:android=#' \
...
javac \
  ...
  -d "$classes_dir" \
  "$generated_java/dev/wzzc/moui/platformshowcase/R.java" \
  "$app_src/java/dev/wzzc/moui/platformshowcase/MainActivity.java"
...
cp "$native_lib" "$apk_work_dir/lib/$abi/libplatform_showcase_android.so"
