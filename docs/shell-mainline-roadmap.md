...
| Host services (IME/clipboard/a11y channel) | Wired in source via `EmbedderHostChannel` + shell adapters |
...
| Renderer selection | Formal `SkiaGpuNative`/`HostGpuSurface` descriptor and `auto`/`skia-gpu`/`skia-raster` selection; `NativeRendererSelection` carries `surface_route` | Product default: all native platforms `gpu_promoted=true`; `auto` selects GPU when available, raster on explicit request or recovery fallback |
...
| Mobile runtime smoke | Recorder writes optional `renderer` (+ pending `gpuPromotionEvidence` skeleton when `gpuPromoted=true`, `claimed=false`) | iOS Simulator CG Release N UIKit shell **`passed`** (Metal), canonical SwiftUI re-run pending; Android CG **`partial`** (`vulkan-gpu` + `gpuAvailable=true`, NDK28 libc++); HarmonyOS HVD CG **`partial`** (`egl-gpu` + `gpuAvailable=true`). Emulator install/verify: `docs/android-support.md` / `docs/harmonyos-support.md`. |
...
`--renderer auto|skia-gpu|skia-raster`. The generated `shell-build.json` and
...
## Shell Embedding Host Bridge

`moui_shell/embedding` owns generation, epoch, frame coalescing, request
correlation, and neutral JSON envelopes. MoUI's private
`backend/internal/embedded_runtime_session` package owns the
`EmbeddingHostBridge`, IME/semantics conversion, and service dispatch. Android
JNI, iOS Obj-C++, and HarmonyOS NAPI only translate this common wire contract
into native platform services. Clipboard responses keep their request id and
surface generation; responses arriving after disposal are ignored.
...
`scripts/record-shell-runtime-smoke.mjs` supports `android`, `ios`, and
