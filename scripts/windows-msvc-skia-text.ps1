...
"@ | Set-Content -LiteralPath $textPkg -NoNewline
  ...
  $cmdFile = Join-Path ([System.IO.Path]::GetTempPath()) "skia-mbt-text-$PID.cmd"
  $traceLine = if ($Trace) { "set SKIA_MBT_TEXT_TRACE=1" } else { "rem SKIA_MBT_TEXT_TRACE not enabled" }
