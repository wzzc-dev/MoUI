...
2. Async image diagnostics
   - Current status: renderer-neutral lifecycle records model loading, ready,
     ...
     and native hosts install baseline-guarded callbacks plus optional
     provider-owned loader hooks after present baseline; the host image repaint
   ...
   - Done when: both host source tests and native provider/platform evidence are
     ...
     and host completion-source routing are current, but they do not yet prove
     that every native renderer has a real out-of-render async loader.
