...
internal class MoUISurfaceView(
    context: Context,
    private val pluginCapabilities: MoUIShellPluginCapabilities,
) : SurfaceView(context) {
        ...
        try {
            ...
            val (updates, generation) = when (value) {
                ...
                is JSONObject -> {
                    ...
                    if (schemaVersion != HOST_WIRE_SCHEMA_VERSION) {
                        Log.e(LOG_TAG, "unsupported shell host wire schema=$schemaVersion")
                        return
                    }
                ...
                else -> throw JSONException("shell host updates must be an array or object")
            }
            if (generation != null &&
            ...
            for (index in 0 until updates.length()) {
                runCatching {
                    ...
                    Log.e(LOG_TAG, "invalid shell host update at index=$index", error)
                }
            }
        } catch (error: JSONException) {
            Log.e(LOG_TAG, "invalid shell host update envelope", error)
        }
    }
    ...
    private fun applyHostUpdate(
        ...
        when (update.optString("kind")) {
            ...
            "diagnostic" -> Log.i(
                LOG_TAG,
                update.optString("payload", "moui-shell diagnostic"),
            )
            ...
            else -> Log.w(LOG_TAG, "unknown shell host update kind=${update.optString("kind")}")
        }
    }
    ...
    private fun applyIme(payload: JSONObject) {
        ...
        Log.i(LOG_TAG, "moui-shell service ime state enabled=$imeEnabled")
        if (nextEnabled) {
            requestFocus()
    ...
    private inner class MoUIInputConnection : BaseInputConnection(this@MoUISurfaceView, true) {
        override fun getEditable(): Editable = this@MoUISurfaceView.editable

        override fun commitText(text: CharSequence, newCursorPosition: Int): Boolean {
            val start = max(0, Selection.getSelectionStart(this@MoUISurfaceView.editable))
            val end = max(0, Selection.getSelectionEnd(this@MoUISurfaceView.editable))
            ...
            composing = false
            Log.i(LOG_TAG, "moui-shell service ime edit kind=commit result=${handled.asInt()}")
