...
directly, then restores both package files. The executable output must include
the final smoke-test success marker so CI proves the real backend path reached
the end of the test.
...
if [[ $enable_skparagraph -eq 1 ]]; then
  ...
  if [[ $require_skparagraph -eq 0 && \
    ...
    # shared builds ship no libskparagraph.so). Emit no SKPARAGRAPH defines and
    # no -lskparagraph flags; use --require-skparagraph to fail instead.
...
if [[ $enable_skparagraph -eq 1 ]]; then
  ...
  if [[ "$skia_provider" == "release" || -n "${MOUI_SKIA_RELEASE_TAG:-}" ]]; then
...
bash "$moui_skia_root/scripts/configure-linux-native-pkg.sh" "${configure_args[@]}" >/dev/null
...
EOF
echo "Wrote temporary scripts/native_smoke/moon.pkg with Linux Skia executable link flags."
