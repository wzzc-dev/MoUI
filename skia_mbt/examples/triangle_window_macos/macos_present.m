...
static const int32_t SKIA_MBT_PRESENT_OK = 0;
static const int32_t SKIA_MBT_PRESENT_BAD_VIEW = 1;
static const int32_t SKIA_MBT_PRESENT_BAD_DIMENSIONS = 2;
static const int32_t SKIA_MBT_PRESENT_BAD_PIXELS = 3;
static const int32_t SKIA_MBT_PRESENT_ALLOC_FAILED = 4;

static NSString *const SKIA_MBT_PRESENT_IMAGE_VIEW_ID = @"skia_mbt_triangle_present_image_view";

static int64_t skia_mbt_expected_pixel_bytes(int32_t width, int32_t height) {
...
MOONBIT_FFI_EXPORT
int32_t skia_mbt_macos_present_rgba_to_window(uint64_t raw_view_handle,
                                              int32_t width, int32_t height,
                                              int32_t row_bytes,
                                              const uint8_t *pixels,
                                              int32_t pixels_len) {
  if (raw_view_handle == 0) {
    return SKIA_MBT_PRESENT_BAD_VIEW;
  }
  ...
  for (NSView *subview in view.subviews) {
    if ([subview isKindOfClass:[NSImageView class]] &&
        [subview.identifier isEqualToString:SKIA_MBT_PRESENT_IMAGE_VIEW_ID]) {
  ...
  if (image_view == nil) {
    ...
    image_view.identifier = SKIA_MBT_PRESENT_IMAGE_VIEW_ID;
