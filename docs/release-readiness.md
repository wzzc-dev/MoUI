...
2. Async image diagnostics
   ...
   - Done when: true native async loader completion sources are either
     implemented or explicitly scoped out of the preview handoff.
     Renderer-local diagnostics, provider callback forwarding, and host
     baseline-guarded redraw scheduling are current, but they do not prove that
     every native renderer has an out-of-render async completion source.
