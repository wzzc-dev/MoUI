...
Unix ASan smoke dry-run, native capability contract, FFI ownership, FFI borrow
annotations, native fallback parity, platform status, native smoke log
verification, and real-Skia artifact verification are all represented.
...
- The native smoke log passed `scripts/verify-native-smoke-log.*`, including
  ...
  non-finite path replay skip and zero-cache-touch proof, canvas command replay,
  ...
  non-finite path replay skip and zero-cache-touch proof,
  ...
  proof, non-finite filter validation, and shader/filter resource planning.
