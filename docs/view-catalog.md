...
| `selection_toolbar` | `views/data_display.mbt` | ButtonStyle/text/surface | Group with selected state | `views/views_test.mbt` | Showcase Data, Data Table | Controlled selected-row summary with bulk action buttons and optional clear action. Apps usually pass `SelectionState::count()` and keep selected row ids and action effects in the app model. |
...
| `selectable_list` | `views/data_display.mbt` | Surface/text/ButtonStyle | List with selected/disabled list items | `views/views_test.mbt` | Showcase Data | Controlled application data list built from `SelectableListItem` descriptors, title/subtitle/detail rows, selected id, disabled rows, typed item messages, and an empty state. Filtering, grouping, and selected-id storage stay app-owned. |
| `pagination` | `views/data_display.mbt` | ButtonStyle/text | Group with disabled edge buttons | `views/views_test.mbt` | Showcase Data, Data Table | Controlled pagination bar; `PaginationState` can clamp the page and expose slicing bounds, while page indexing and data slicing still stay app-owned. |
