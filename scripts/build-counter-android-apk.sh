...
usage() {
  cat <<'USAGE'
Usage: scripts/build-counter-android-apk.sh [options]

Build the experimental Counter Android debug APK.
...
Options:
  ...
  --build-dir <dir>   Working directory, default artifacts/android/counter.
  --output <apk>      APK output, default artifacts/android/counter/app-debug.apk.
...
api_level="23"
compile_sdk="35"
build_dir="$repo_root/artifacts/android/counter"
output_apk=""
fallback_skia=0
prepare_only=0
...
log "Generating MoonBit native C for examples/counter/android_skia"
(
  cd "$repo_root"
  MOUI_SKIA_DISABLE_PREBUILD_SKIA=1 \
  MOUI_PDFIUM_DISABLE_PREBUILD_PDFIUM=1 \
    moon build examples/counter/android_skia --target native --target-dir "$moon_target_dir"
)

moonbit_c="$moon_target_dir/native/debug/build/examples/counter/android_skia/android_skia.c"
if [ ! -f "$moonbit_c" ]; then
...
cmake_args=(
  -S "$repo_root/examples/counter/android_app/app/src/main/cpp"
  -B "$cmake_build_dir"
  ...
  "-DMOUI_COUNTER_ANDROID_MOONBIT_C=$moonbit_c"
...
native_lib="$(find "$cmake_build_dir" -name libmoui_counter_android.so -type f | head -n 1)"
if [ -z "$native_lib" ]; then
  echo "native library was not produced under $cmake_build_dir" >&2
  exit 1
fi

app_src="$repo_root/examples/counter/android_app/app/src/main"
...
sed 's#<manifest xmlns:android=#<manifest package="dev.wzzc.moui.counter" xmlns:android=#' \
...
javac \
  ...
  -d "$classes_dir" \
  "$generated_java/dev/wzzc/moui/counter/R.java" \
  "$app_src/java/dev/wzzc/moui/counter/MainActivity.java"
...
cp "$native_lib" "$apk_work_dir/lib/$abi/libmoui_counter_android.so"
