...
usage() {
  cat <<'USAGE'
Usage: scripts/build-component-gallery-ios-app.sh [options]

Build the experimental Component Gallery iOS Simulator .app bundle.
...
Options:
  ...
  --build-dir <dir>         Working directory. Default artifacts/ios/component_gallery.
  --output <app>            App bundle path. Default artifacts/ios/component_gallery/ComponentGallery.app.
...
build_dir="$repo_root/artifacts/ios/component_gallery"
output_app=""
fallback_skia=0
prepare_only=0
...
log() {
  printf '[moui-component-gallery-ios] %s\n' "$*"
}
...
if [ -z "$output_app" ]; then
  output_app="$build_dir/ComponentGallery.app"
fi
...
log "Generating MoonBit native C for examples/component_gallery/ios"
(
  cd "$repo_root"
  MOUI_SKIA_DISABLE_PREBUILD_SKIA=1 \
  MOUI_PDFIUM_DISABLE_PREBUILD_PDFIUM=1 \
    moon build examples/component_gallery/ios --target native --target-dir "$moon_target_dir"
)

moonbit_c="$moon_target_dir/native/debug/build/examples/component_gallery/ios/ios.c"
if [ ! -f "$moonbit_c" ]; then
...
include_flags=(
  -I "$repo_root/examples/component_gallery/ios_app/include"
  -I "$moon_home/include"
  -I "$repo_root/moui_skia/native"
)
objects=()
compile_c "$moonbit_c" "$obj_dir/ios.o" -Dmain=moui_component_gallery_ios_moonbit_generated_main
...
compile_c "$repo_root/examples/component_gallery/ios_app/moui_ios_compat.c" "$obj_dir/moui_ios_compat.o"
compile_objcxx "$repo_root/examples/component_gallery/ios_app/main.mm" "$obj_dir/main.o"
...
executable_name="ComponentGallery"
...
cp "$repo_root/examples/component_gallery/ios_app/Info.plist" "$app_bundle/Info.plist"
