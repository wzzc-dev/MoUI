...
the closed window from the tracked repaint table. `HostImageResourceCompletionSource`
now gives native provider/platform loaders a host-layer boundary to publish
revisioned completion snapshots into that routing path; it does not decode
images, mutate renderer caches, or prove matching-host loader/runtime behavior.
Skia now exposes a
...
redraw for post-present image revisions. Provider/platform async loader apply
ports and matching-host runtime evidence remain follow-up work. Skia renderer-local
