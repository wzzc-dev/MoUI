...
| Gradient | partial | ready | partial | Skia now renders linear and radial gradient brushes for rounded rects and paths; WGPU/Web keep linear gradients ready while radial rounded fill/stroke brushes use deterministic center-edge gradient fallback paths until true radial shaders land. |
