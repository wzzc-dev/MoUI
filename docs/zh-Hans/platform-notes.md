...
Web 通过同一 registry/slot 路径创建主窗口，`WebAppOptions` 由 `@web.entry` 捕获并支持 resolver-backed `OpenWindow` 请求。原生宿主核心通过同一路径创建平台窗口，但不自行选择具体渲染器族。应用入口点向 `@runtime.run_app` 提供有序 `RendererFactory` 和一个平台 `PlatformEntry`。已解析窗口创建中立 `SurfaceContext`，解析出 `WindowRenderer`，随后注册 `HostRuntimeDriver`、平台绑定和平台 slot，并按 `WindowId` 路由重绘、事件、上下文菜单、宿主服务 completion、IME 同步和释放。如果没有 scene resolver，宿主会用共享 unavailable-resolver 响应拒绝 `OpenWindow`。

原生渲染器选择属于应用组合，不是 host-core options 字段。Skia 主线使用 `render/skia.from_env()`，原生 WGPU 诊断使用 `render/wgpu.native(...)`。Android、iOS 和 HarmonyOS 使用 `wzzc-dev/window` 作为嵌入运行时后端的 embedder。template 通过 `EventLoop` 将 `HostCmd` 交给匹配的 `*EmbeddedRuntimeBackend`，后者装配 MoUI runtime session 和中立宿主表面。`wzzc-dev/window/internal/embedded_dispatch` 只做三端物理 callback dispatch；平台 `HostCmd` adapter 只保留原生 payload 解码和 nominal `ApplicationHandler`/`Window` effects。MoUI 的 `EmbeddedWindowCoordinator` 统一拥有 lifecycle、surface generation、primary-window、detach 与 exit intent；`embedded_runtime` 提供 session/renderer/IME/semantics/platform views/transport 能力，`FrameCoordinator` 统一 redraw/completion/image repaint 时序。lifecycle、surface 和 input 不得绕过该路线。
...
- Sun factory 会显式报告渲染器侧平台视图像素接线。macOS、Windows 和 Linux Sun 在其 `WindowRenderer` 包装器把离屏平台视图像素转发进 Sun 帧时报告 `renderer_platform_view_pixels=SunRasterRenderer.draw_platform_view_pixels`。这是应用层 renderer composition 接线；在提出更宽的平台运行时就绪声明前，仍需要匹配宿主运行时 smoke。
