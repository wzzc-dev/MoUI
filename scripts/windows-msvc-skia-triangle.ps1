...
"@ | Set-Content -LiteralPath $trianglePkg -NoNewline
  ...
  $cmdFile = Join-Path ([System.IO.Path]::GetTempPath()) "skia-mbt-triangle-$PID.cmd"
  $traceLine = if ($Trace) { "set SKIA_MBT_TRIANGLE_TRACE=1" } else { "rem SKIA_MBT_TRIANGLE_TRACE not enabled" }
