...
| Devtool counters | Core inspector snapshots expose runtime, layout, semantics, frame, render command, TEA dispatch/update/message-queue/effect-plan/effect-kind counters, latest effect summaries, subscription-plan counters/summaries, and app subscription lifecycle counters; render snapshots also report open clip/layer/filter scopes and unbalanced pops, and Showcase Diagnostics surfaces the render command/scope counters with app-test coverage. | ready for command-level diagnostics |
