...
usage() {
  cat <<'USAGE'
Usage: scripts/build-counter-ios-app.sh [options]

Build the experimental Counter iOS Simulator .app bundle.
...
Options:
  ...
  --build-dir <dir>         Working directory. Default artifacts/ios/counter.
  --output <app>            App bundle path. Default artifacts/ios/counter/MoUICounter.app.
...
build_dir="$repo_root/artifacts/ios/counter"
output_app=""
fallback_skia=0
prepare_only=0
...
if [ -z "$output_app" ]; then
  output_app="$build_dir/MoUICounter.app"
fi
...
log "Generating MoonBit native C for examples/counter/ios_skia"
(
  cd "$repo_root"
  MOUI_SKIA_DISABLE_PREBUILD_SKIA=1 \
  MOUI_PDFIUM_DISABLE_PREBUILD_PDFIUM=1 \
    moon build examples/counter/ios_skia --target native --target-dir "$moon_target_dir"
)

moonbit_c="$moon_target_dir/native/debug/build/examples/counter/ios_skia/ios_skia.c"
if [ ! -f "$moonbit_c" ]; then
...
include_flags=(
  -I "$repo_root/examples/counter/ios_app/include"
  -I "$moon_home/include"
  -I "$repo_root/moui_skia/native"
)
objects=()
compile_c "$moonbit_c" "$obj_dir/ios_skia.o" -Dmain=moui_ios_moonbit_generated_main
...
compile_c "$repo_root/examples/counter/ios_app/moui_ios_compat.c" "$obj_dir/moui_ios_compat.o"
compile_objcxx "$repo_root/examples/counter/ios_app/main.mm" "$obj_dir/main.o"
...
cp "$repo_root/examples/counter/ios_app/Info.plist" "$app_bundle/Info.plist"
