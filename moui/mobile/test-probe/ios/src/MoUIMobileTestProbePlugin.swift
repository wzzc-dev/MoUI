...
private final class ProbeState {
  ...
  func increment(_ counter: ProbeCounter) {
    ...
    let value = counters
    lock.unlock()
    NSLog("moui-mobile test-probe snapshot=%@", Self.encode(value))
  }
...
private final class ProbePlatformView: UILabel {
  var eventSink: MOUIMobilePlatformViewEventSink?
  var lastSize: CGSize?
  var sentReady = false
  ...
  override init(frame: CGRect) {
    ...
    accessibilityIdentifier = "moui-mobile-test-probe-platform-view"
...
private final class ProbePlatformViewFactory: MOUIMobilePlatformViewFactory {
...
private final class ProbePendingTask: MOUIMobileHostChannelTask {
  enum Kind {
    case cancel
  ...
  private let completion: MOUIMobileHostChannelCompletion
  private let kind: Kind

  init(completion: MOUIMobileHostChannelCompletion, kind: Kind) {
...
private final class ProbeHostChannelHandler: MOUIMobileHostChannelHandler {
  func handle(
...
private final class ProbeServiceSmoke {
  ...
  private var subscription: MOUIMobilePluginSubscription?

  func install(capabilities: MOUIMobilePluginCapabilities) {
    ...
    subscription = capabilities.semantics.observe { [weak self, weak capabilities] snapshot, runtimeInput in
      ...
      NSLog("moui-mobile service smoke begin")
      ...
      if imeCommitAccepted { NSLog("moui-mobile service ime edit kind=commit") }
      ...
      if copyAccepted { NSLog("moui-mobile service smoke copy") }
      ...
      if pasteAccepted { NSLog("moui-mobile service smoke paste") }
      let cutAccepted = runtimeInput.dispatchCommand(kind: 1)
      accepted = cutAccepted && accepted
      if cutAccepted { NSLog("moui-mobile service smoke cut") }
      ...
      if actionFocusAccepted {
        NSLog("moui-mobile service accessibility focus id=%d", action.elementIdentifier)
      }
      ...
      if actionActivateAccepted {
        NSLog("moui-mobile service accessibility action=activate id=%d", action.elementIdentifier)
      }
      if accepted && runtimeInput.isActive {
        ProbeState.shared.increment(.serviceSmokeCompleted)
      }
      NSLog("moui-mobile service smoke end accepted=%d", accepted ? 1 : 0)
    }
  }
}

public enum MoUIMobileTestProbePlugin: MOUIMobilePlugin {
  public static let identifier = pluginIdentifier

  public static func install(in registry: MOUIMobilePluginRegistry) {
