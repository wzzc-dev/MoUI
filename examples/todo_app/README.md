...
`examples/todo_app` 是可复用的 Todo 应用包。当前 Web 主线只使用 `wasm-gc`，不再保留 JS target Web 示例；Todo 的可运行入口目前是 Windows native。
...
Web host 已切到单一路径：`examples/counter_web_wasm --target wasm-gc`。Todo 如果需要 Web 入口，应按 `examples/counter_web_wasm` 的模式新增 `todo_web_wasm`，直接复用 `backend/web` 和 `render/webgpu`，不要走 JS target fallback。
