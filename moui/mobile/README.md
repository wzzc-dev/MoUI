...
so an application project can build Android/iOS shells without depending on the
MoUI repository checkout.

The reusable pieces live here:

- `scripts/mobile/prepare-native-build.mjs`: generates MoonBit native C,
  resolves Skia provider flags, and writes Android/iOS build inputs.
...
iOS Xcode project, and MoonBit platform entrypoint packages. The MoUI package
owns only the reusable shell/build templates. The repository example contracts
in `mobile/build-contracts.json` are compatibility defaults for MoUI's own
Counter and Component Gallery examples; external apps should put their native
contract in their own `mobile.json`.
...
Android/iOS runtime claim still requires a non-fallback build plus matching
emulator/simulator or device smoke evidence.
