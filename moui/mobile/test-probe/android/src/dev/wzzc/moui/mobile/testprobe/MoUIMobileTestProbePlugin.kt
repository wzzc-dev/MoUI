package dev.wzzc.moui.mobile.testprobe
...
import dev.wzzc.moui.mobile.MoUIHostServiceCompletion
import dev.wzzc.moui.mobile.MoUIHostServiceHandler
import dev.wzzc.moui.mobile.MoUIHostServiceRequest
import dev.wzzc.moui.mobile.MoUIHostServiceTask
import dev.wzzc.moui.mobile.MoUIHostServices
import dev.wzzc.moui.mobile.MoUIMobilePlugin
import dev.wzzc.moui.mobile.MoUIMobilePluginCapabilities
import dev.wzzc.moui.mobile.MoUIPluginSubscription
import dev.wzzc.moui.mobile.MoUIPlatformViewEventSink
import dev.wzzc.moui.mobile.MoUIPlatformViewFactory
import dev.wzzc.moui.mobile.MoUIPlatformViewPlacement
import dev.wzzc.moui.mobile.MoUIPlatformViews
import org.json.JSONObject
import java.util.concurrent.atomic.AtomicBoolean
import java.util.concurrent.atomic.AtomicInteger

private const val PLUGIN_ID = "dev.wzzc.moui.mobile.test-probe"
private const val PLATFORM_VIEW_KIND = "$PLUGIN_ID.view"
private const val HOST_CHANNEL = "$PLUGIN_ID.channel"
private const val TEST_PROBE_GATE = "moui.mobile.testProbe"
private const val SERVICE_TEXT_LABEL = "Service probe text"
private const val SERVICE_ACTION_LABEL = "Activate service probe"
private const val SERVICE_PROBE_TEXT = "ime-mobile-probe"
...
private object ProbeState {
    ...
    fun increment(counter: AtomicInteger) {
        ...
        Log.i(LOG_TAG, "moui-mobile test-probe snapshot=${snapshot()}")
    }
...
private object ProbeServiceSmoke {
    ...
    fun install(context: Context, capabilities: MoUIMobilePluginCapabilities) {
        ...
        subscription = capabilities.semantics.observe { snapshot, runtimeInput ->
            ...
            Log.i(LOG_TAG, "moui-mobile service smoke begin")
            ...
            if (imeCommitAccepted) Log.i(LOG_TAG, "moui-mobile service ime edit kind=commit")
            ...
            if (copyAccepted) Log.i(LOG_TAG, "moui-mobile service smoke copy")
            ...
            if (pasteAccepted) Log.i(LOG_TAG, "moui-mobile service smoke paste")
            val cutAccepted = runtimeInput.dispatchCommand(1)
            accepted = cutAccepted && accepted
            if (cutAccepted) Log.i(LOG_TAG, "moui-mobile service smoke cut")
            ...
            if (actionFocusAccepted) {
                Log.i(LOG_TAG, "moui-mobile service accessibility focus id=${action.elementId}")
            }
            ...
            if (actionActivateAccepted) {
                Log.i(LOG_TAG, "moui-mobile service accessibility action=activate id=${action.elementId}")
            }
            if (accepted && runtimeInput.isActive) {
                ProbeState.increment(ProbeState.serviceSmokeCompleted)
            }
            Log.i(LOG_TAG, "moui-mobile service smoke end accepted=${if (accepted) 1 else 0}")
        }
    }
}
...
class MoUIMobileTestProbePlugin : MoUIMobilePlugin {
    ...
    override fun install(context: Context, capabilities: MoUIMobilePluginCapabilities) {
        install(context)
