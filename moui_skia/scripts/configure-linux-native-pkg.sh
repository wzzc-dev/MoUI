...
usage() {
  cat <<'EOF'
Usage: scripts/configure-linux-native-pkg.sh --skia-include PATH --skia-lib-dir PATH [options]

Generates the native/moon.pkg contents needed to link the native package against
an existing Linux Skia build. By default the generated package is printed to
...
if [[ $enable_skparagraph -eq 1 ]]; then
  link_flags="$link_flags -L$lib_path -Wl,--start-group -lskparagraph -lskshaper -lskunicode_icu -lskunicode_core -lharfbuzz -licu -Wl,--end-group"
fi
...
if [[ $check_config -eq 1 ]]; then
  ...
  if ! diff -u "$resolved_output_path" <(printf '%s\n' "$generated_config"); then
    echo "native package file does not match generated Linux Skia link config: $resolved_output_path" >&2
    exit 1
  fi
  echo "Verified $resolved_output_path matches generated Linux Skia link config."
  exit 0
fi
...
if [[ $write_config -eq 1 ]]; then
  ...
  echo "Wrote Linux Skia link config to $resolved_output_path"
  exit 0
fi
