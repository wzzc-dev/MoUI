...
#ifndef MOUI_EMBEDDING_APP_ARG
#define MOUI_EMBEDDING_APP_ARG "moui-shell-harmonyos"
#endif

#ifndef MOUI_EMBEDDING_APP_ID
#define MOUI_EMBEDDING_APP_ID "unknown"
#endif

#ifndef MOUI_EMBEDDING_RENDERER_REQUESTED
#define MOUI_EMBEDDING_RENDERER_REQUESTED "auto"
#endif

#ifndef MOUI_EMBEDDING_RENDERER_SELECTED
#define MOUI_EMBEDDING_RENDERER_SELECTED MOUI_EMBEDDING_RENDERER_REQUESTED
#endif

#ifndef MOUI_EMBEDDING_SMOKE_ATTACH_SURFACE
#define MOUI_EMBEDDING_SMOKE_ATTACH_SURFACE                                       \
  moui_embedding_harmonyos_attach_surface_for_smoke
#endif

#ifndef MOUI_EMBEDDING_SMOKE_RENDER_FRAME
#define MOUI_EMBEDDING_SMOKE_RENDER_FRAME                                         \
  moui_embedding_harmonyos_render_frame_for_smoke
#endif
...
const moui_embedding_api_v1 *g_runtime_api = nullptr;
...
int32_t g_initialize_result = MOUI_EMBEDDING_API_ERROR_NOT_INITIALIZED_V1;
...
public:
  explicit OwnedUtf8Buffer(moui_embedding_utf8_buffer_v1 value) : value_(value) {}
  ...
  const moui_embedding_utf8_buffer_v1 &get() const { return value_; }

private:
  moui_embedding_utf8_buffer_v1 value_;
};

moui_embedding_utf8_view_v1 utf8_view(const char *value) {
...
moui_embedding_utf8_view_v1 utf8_view(const std::string &value) {
  return {
...
std::string consume_utf8_buffer(moui_embedding_utf8_buffer_v1 raw,
                                const char *operation) {
  OwnedUtf8Buffer buffer(raw);
  const auto &value = buffer.get();
  if (value.status != MOUI_EMBEDDING_API_STATUS_OK_V1 ||
      value.data == nullptr || value.release == nullptr) {
    log_warn(
        "moui-shell ABI buffer failure operation=%{public}s status=%{public}d",
...
bool ensure_runtime_initialized() {
  ...
  std::call_once(g_runtime_once, [] {
    g_runtime_api = moui_embedding_get_api_v1();
    if (!moui_embedding_api_v1_is_compatible(g_runtime_api)) {
      g_runtime_api = nullptr;
      g_initialize_result = MOUI_EMBEDDING_API_ERROR_INITIALIZATION_FAILED_V1;
      log_warn("moui-shell runtime ABI v1 negotiation failed app=%{public}s",
               MOUI_EMBEDDING_APP_ID);
      return;
    }
    g_initialize_result =
        g_runtime_api->initialize(utf8_view(MOUI_EMBEDDING_APP_ARG));
    if (g_initialize_result != MOUI_EMBEDDING_API_RESULT_TRUE_V1) {
      log_warn("moui-shell runtime initialize failed app=%{public}s "
               "status=%{public}d",
               MOUI_EMBEDDING_APP_ID, g_initialize_result);
      return;
    }
    const int32_t renderer_result = g_runtime_api->configure_renderer(
        utf8_view(MOUI_EMBEDDING_RENDERER_REQUESTED));
    const std::string status = consume_utf8_buffer(
        g_runtime_api->renderer_status_json(), "renderer-status");
    log_info("moui-shell runtime initialized app=%{public}s "
             "renderer-requested=%{public}s renderer-selected=%{public}s",
             MOUI_EMBEDDING_APP_ID, MOUI_EMBEDDING_RENDERER_REQUESTED,
             MOUI_EMBEDDING_RENDERER_SELECTED);
    log_info("moui-shell renderer configure requested=%{public}s "
             "status=%{public}d detail=%{public}s",
             MOUI_EMBEDDING_RENDERER_REQUESTED, renderer_result, status.c_str());
  });
  return g_runtime_api != nullptr &&
         g_initialize_result == MOUI_EMBEDDING_API_RESULT_TRUE_V1;
}

bool boolean_result(const char *operation, int32_t result) {
  if (result < MOUI_EMBEDDING_API_STATUS_OK_V1) {
    log_warn(
        "moui-shell ABI call failed operation=%{public}s status=%{public}d",
        operation, result);
  }
  return result == MOUI_EMBEDDING_API_RESULT_TRUE_V1;
}
...
bool attach_or_resize(uint64_t surface_handle, int32_t width, int32_t height,
  ...
  log_info("moui-shell XComponent %{public}s width=%{public}d "
           "height=%{public}d scale=%{public}f status=%{public}d",
           same_surface ? "resize" : "attach", width, height, scale, result);
  if (!same_surface) {
    log_info("moui-shell lifecycle attach width=%{public}d height=%{public}d "
             "attached=%{public}d",
             width, height,
             result == MOUI_EMBEDDING_API_RESULT_TRUE_V1 ? 1 : 0);
  } else {
    log_info("moui-shell resize width=%{public}d height=%{public}d "
             "result=%{public}d",
             width, height,
             result == MOUI_EMBEDDING_API_RESULT_TRUE_V1 ? 1 : 0);
  }
...
void on_surface_destroyed(OH_NativeXComponent *component, void *window) {
  ...
  g_surface_handle = 0;
  reset_touch_gesture();
  log_info("moui-shell lifecycle detach reason=surface-destroyed");
}
...
void dispatch_touch_event(OH_NativeXComponent *component, void *window) {
  ...
  log_info(
      "moui-shell input pointer type=%{public}d x=%{public}f y=%{public}f "
  ...
  if (scroll_dispatched) {
    log_info("moui-shell input scroll x=%{public}f y=%{public}f "
             "source=native-xcomponent",
             event.x, event.y);
  }
}
...
napi_value napi_string_from_buffer(napi_env env, moui_embedding_utf8_buffer_v1 raw,
                                   const char *operation) {
  OwnedUtf8Buffer buffer(raw);
  const auto &value = buffer.get();
  napi_value result;
  if (value.status != MOUI_EMBEDDING_API_STATUS_OK_V1 ||
    ...
    log_warn(
        "moui-shell ABI string failure operation=%{public}s status=%{public}d",
        operation, value.status);
...
napi_value napi_complete_clipboard(napi_env env, napi_callback_info info) {
  ...
  const moui_embedding_bytes_view_v1 bytes = {
...
napi_value napi_destroy_application(napi_env env, napi_callback_info info) {
  ...
  log_info("moui-shell application destroy app=%{public}s status=%{public}d",
           MOUI_EMBEDDING_APP_ID, result);
...
napi_value init(napi_env env, napi_value exports) {
  ...
  log_info("NAPI module initialized with Shell Runtime ABI v1");
  return exports;
}
#endif
...
extern "C" int32_t MOUI_EMBEDDING_SMOKE_ATTACH_SURFACE(uint64_t surface_handle,
                                                    int32_t width,
                                                    int32_t height,
                                                    double scale_factor) {
...
extern "C" int32_t MOUI_EMBEDDING_SMOKE_RENDER_FRAME(void) {
  RuntimeCallLock runtime_call_lock(g_runtime_call_mutex);
  if (!ensure_runtime_initialized()) {
...
#define NODE_GYP_MODULE_NAME moui_embedding_harmonyos
#endif
