...
test("HarmonyOS shell exposes generation-scoped generic plugin capabilities", () => {
  const plugins = read("moui/mobile/harmonyos/template/entry/src/main/ets/moui/MoUIPlugins.ets");
  for (const token of [
...
test("HarmonyOS root publishes semantics after applying UI state", () => {
  const root = read("moui/mobile/harmonyos/template/entry/src/main/ets/moui/MoUIRoot.ets");
  before(
    root,
...
test("HarmonyOS Ability maps the unified Want launch option and destroys plugin state", () => {
  const ability = read(
    "moui/mobile/harmonyos/template/entry/src/main/ets/entryability/EntryAbility.ets",
  );
  for (const token of [
    "import Want from '@ohos.app.ability.Want'",
    "const TEST_PROBE_LAUNCH_OPTION: string = 'moui.mobile.testProbe'",
    "onCreate(want: Want",
    "onNewWant(want: Want",
...
test("HarmonyOS test-probe gates and once-fires the stable-label service sequence", () => {
  const probe = read("moui/mobile/test-probe/harmonyos/src/MoUIMobileTestProbePlugin.ets");
  for (const token of [
...
test("production HarmonyOS shell contains no service-probe labels or scripted actions", () => {
  const production = [
    "moui/mobile/harmonyos/template/entry/src/main/ets/moui/MoUIPlugins.ets",
