...
| Text | ready | ready | ready | Skia basic `Font` measurement, font-metric baseline/height, measured caret positions, and positioned glyph-run rendering resolve `FontSpec` family, weight, and style through Skia `FontMgr`/`Font`, with real native renderer smoke evidence; shaping is tracked separately. |
...
| Text shaping | partial | partial | partial | Skia maps `FontSpec` family, weight, and style and returns Skia font-metric baseline/height plus measured prefix caret positions for basic `Font` measurement and positioned glyph-run drawing after linking, but SkShaper/SkParagraph-style shaping, bidi, line breaking, and typography conformance remain follow-up work. |
...
placeholders, and basic text glyph-run pixels while requiring
...
caret positions, and positioned glyph runs for rendering, while complex shaping,
bidi, line
