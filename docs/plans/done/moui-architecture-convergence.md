...
- **Goal**: Converge MoUI package ownership and dependency direction to match
  ...
  `views` own control vocabulary, `backend` platform-neutral contracts
  only, `render/*` a provider-plugin system, platform adapters thin wiring
  over `backend/common`. Preserve Native Skia mainline, Native WGPU
...
Phase A  foundation contracts (no behavior change)
  ...
  A3  scripts/validate-host-import-baseline.mjs +
      ...
      scripts/validate-backend-common-boundary.mjs
...
Phase D  Task 3 — host contract split (ADR 0018)
  D1  moui/runtime: receive HostRuntimeDriver + wall_clock + redraw_scheduler
      + runtime-side subscription source adapters from backend.
  ...
  D3  moui/backend: shrink to Event/HostCmd/services facade/
  ...
  Gate: moon info moui/backend shows no runtime/render symbols;
        moon test moui/backend moui/runtime moui/render --target native;
...
Phase F  Task 5 — platform adapter convergence (ADR 0020)
  F1  moui/backend/common: package owning cross-platform neutral
      ...
      imports core + backend + window value types only. Native pointer,
  ...
  F2  backend/{macos,windows,linux,web,android,ios,harmonyos,wechat}: shrink to
      ...
      `WindowCoordinator` (moui/backend/common) for macos/windows/linux/web;
      embedded-runtime mobile shells converged into
      `moui/backend/common/embedded`; per-platform
      `window_hosted.mbt` is a thin shell.]
  F3  backend/common/*_test.mbt: table-driven tests feed
      (native_type_tag, payload) → expected Event/HostCmd for every
      platform in one place.
  F4  Enforce the Platform Bridge ownership boundary. A later 2026-08-05
      amendment removed similarity/budget accounting entirely: genuine native
      differences stay local, while shared behavior moves to its owning package.
  Gate: validate-backend-common-boundary green;
        ...
        the later boundary-only validator replaced file-level mirror
        similarity so nominal adapters remain legal while duplicate behavior
...
Phase G  docs + repo-local skill + final verification
  ...
  G2  docs/invariants.md: update P3 (core neutral, no control vocab),
      ...
      add renderer-provider governance and the temporary adapter-budget row
      that the 2026-08-05 amendment later removed.
  G3  docs/testing.md: register the four new validators + platform profile
      expectation + the then-current adapter policy, later replaced by the
      budget-free Platform Bridge boundary gate.
  ...
  G7  Final: moon info moui/core moui/views moui/backend — capture API
...
- Phase F does not change any platform's observable behavior; the table-driven
  transformers reproduce the exact Event/HostCmd mappings each platform
...
- Any growth in API surface or renderer central coupling requires the matching
  governance review. Platform behavior duplication is fixed in code rather
  than accepted through an allowlist.
...
| 2026-07-30 | **Phases B–F gate-level complete**. All four architecture validators enforcing and green (core theme, host imports, Platform Bridge, renderer provider). Gate tests pass: core 37/37, views 48/48, host+runtime+render 225/225, platform_bridge 12/12. `moon check --target all` clean. Phase G (documentation) is the remaining work. |
| 2026-08-02 | **Phase G complete — plan done, ready to archive**. G1–G6 landed: `architecture-map.md` dependency diagram/ownership cheat sheet rewritten for ADR 0018/0019/0020 (host contracts-only, runtime-owned `HostRuntimeDriver`, `platform_bridge`, `RendererProviderBinding` composition, Sun experimental row); `testing.md` registered the four architecture validators and platform profile expectation; `skills/moui-framework-development-skill/SKILL.md` added convergence pointers; five docs + zh-Hans mirrors aligned with ADR 0017–0020/0023. The temporary adapter-budget policy recorded at the time was removed by the 2026-08-05 ADR 0020 amendment in favor of direct behavioral convergence. G7 final gate green: static trio ok, `moon info` no API drift, all four profiles pass (`pr`, `theme`, `daily`, `platform`). |
