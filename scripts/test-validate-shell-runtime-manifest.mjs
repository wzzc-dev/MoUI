...
const tmp = mkdtempSync(join(tmpdir(), "moui-shell-runtime-manifest-"));
const validator = "scripts/validate-shell-runtime-manifest.mjs";
const recorder = "scripts/record-shell-runtime-smoke.mjs";
...
const baseManifest = (overrides = {}) => ({
  ...
  build: {
    ...
    command: "scripts/build-shell-ios-app.sh --app showcase",
  },
  artifacts: {
    screenshotBefore: "artifacts/shell-runtime/ios/showcase/screenshot-before.png",
    screenshot: "artifacts/shell-runtime/ios/showcase/screenshot.png",
    log: "artifacts/shell-runtime/ios/showcase/runtime.log",
  },
  screenshot: {
    width: 390,
    height: 844,
...
assert(
  !hasIosApplicationLog(
    "log[12:0] args: eventMessage CONTAINS 'moui-shell lifecycle detach'",
    "MoUIShowcase",
...
assert(
  hasIosApplicationLog(
    "MoUIShowcase[42:7] moui-shell lifecycle detach app=showcase",
    "MoUIShowcase",
    "moui-shell lifecycle detach",
  ),
...
assert(
  hasMobileResizeTransition([
    "moui-shell lifecycle attach width=1206 height=2622",
    "moui-shell resize width=2622 height=1206",
  ].join("\n")),
  "shell resize evidence should require two distinct physical sizes",
);
assert(
  !hasMobileResizeTransition([
    "moui-shell lifecycle attach width=1206 height=2622",
    "moui-shell resize width=1206 height=2622",
  ].join("\n")),
  "a duplicate initial surface callback must not count as resize evidence",
);
assert(
  hasMobileTextClipboardRoundTrip([
    "moui-shell service clipboard complete operation=write-text accepted=1",
    "moui-shell service clipboard complete operation=read-text accepted=true",
  ].join("\n"))
    && !hasMobileTextClipboardRoundTrip(
      "moui-shell service clipboard complete operation=write-text",
    ),
  "shell clipboard evidence should require system write and read completion",
);
assert(
  !hasMobileTextClipboardRoundTrip([
    "moui-shell service clipboard complete operation=write-text accepted=1",
    "moui-shell service clipboard complete operation=read-text accepted=0",
  ].join("\n")),
  "a stale-generation clipboard completion must not count as a round trip",
);
const probeSnapshot = parseMobileTestProbeSnapshot([
  "moui-shell test-probe snapshot={truncated",
  'moui-shell test-probe snapshot={"platformViewCreate":1,"platformViewResize":2,"platformViewClip":1,"platformViewEvent":2,"platformViewDispose":1,"hostChannelSuccess":1,"hostChannelError":1,"hostChannelCancel":1,"hostChannelExactlyOnce":1,"hostChannelLateAfterDispose":1,"serviceSmokeFired":1,"serviceSmokeCompleted":1}',
  'moui-shell test-probe snapshot={"platformViewCreate":2,"platformViewResize":3,"platformViewClip":2,"platformViewEvent":3,"platformViewDispose":1,"hostChannelSuccess":2,"hostChannelError":1,"hostChannelCancel":1,"hostChannelExactlyOnce":1,"hostChannelLateAfterDispose":1,"serviceSmokeFired":1,"serviceSmokeCompleted":1}',
].join("\n"));
assert(
...
assert(
  parseMobileTestProbeSnapshot(
    'moui-shell test-probe snapshot={"platformViewCreate":-1}',
  ) === null,
  "test-probe parser should reject incomplete or invalid counters",
);
const semanticsProbePlan = parseMobileServiceProbePlan([
  "moui-shell service accessibility tree nodes=12",
  "moui-shell service probe plan textField=120,240 action=180,300 textFieldId=7 actionId=9 density=2.75",
  "moui-shell service probe plan textField=130,250 action=190,310 textFieldId=7 actionId=9 density=2.75",
].join("\n"));
assert(
...
const rendererLog = [
  'moui-shell renderer configure requested=auto ok=1 status={"platform":"ios","requested":"auto","selected":"skia-gpu-native","surfaceRoute":"metal-gpu","gpuAvailable":true,"gpuPromoted":true,"fallbackReason":null}',
  'moui-shell lifecycle attach app=counter width=1206 height=2622 attached=1',
].join("\n");
const parsedRenderer = parseShellRendererStatus(rendererLog);
assert(
  ...
  "runtime renderer configure status should map into the shell-runtime renderer block",
);
assert(
  parseShellRendererStatus("no configure line") === null,
...
assert(
  ...
  "shell-build.json renderer should normalize to the runtime schema",
);

console.log("shell runtime manifest validator tests passed");
