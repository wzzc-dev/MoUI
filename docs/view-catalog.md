...
| `stack` | `views/layout/stack.mbt` | Child/modifier based | Children preserved | `views/views_test.mbt` | General layout | Pure same-origin layout primitive. It does not own presentation ordering, hit testing, or dismissal. |
| `overlay_host` | `views/presentation.mbt` | `PresentationSpec` list | Ordered base + presentation children | `views/presentation_test.mbt` | Showcase, app examples | Sole presentation composition entrypoint. Ordered active specs are laid out back-to-front; modal barriers block background input, Escape is top-most-first, and host policy is available to neutral backend adapters. |
...
| `dialog` | `views/dialog_shell.mbt` | SurfaceStyle/ButtonStyle | Dialog content | `views/views_test.mbt`, Showcase tests | Showcase Navigation Shell | Reusable dialog content. Present it with `PresentationSpec(kind=Dialog)` inside `overlay_host`. |
| `sheet` | `views/popover/sheet.mbt` | Brush, radius, shadow | Sheet content | `views/views_test.mbt` | Showcase | Reusable sheet content. Present it with `PresentationSpec(kind=Sheet)` inside `overlay_host`; no second sheet host exists. |
