...
| --- | --- | --- |
| Daily validation | `sh scripts/dev-check.sh` runs the bounded mainline package tests, API surface guard, checked conformance artifact guard, dedicated checked-artifact validators for capture/Web/platform/renderer-proof manifests, and Web wasm-gc builds for Showcase and Markdown Editor. Design Systems is addon diagnostic coverage through `sh scripts/dev-check.sh --theme-diagnostics`. | ready |
...
| Text system | `docs/text-system.md` documents `TextSystem`, paragraph layout geometry, provider composition, embedded fonts, Skia `skia_text_system()` diagnostic coverage, the optional native SkParagraph route, and shaping gaps; stable and diagnostic text conformance checks pass with paragraph line metrics/selection/hit-test coverage while still not claiming full native SkParagraph/bidi readiness without passed renderer-proof artifacts. | ready with tracked gates |
...
| --- | --- | --- |
| Daily baseline | Bounded package checks, API surface guardrails, checked evidence validators, and Web wasm-gc example builds pass. | `sh scripts/dev-check.sh` |
| Public API audit | Generated interfaces reviewed after public API changes and current package budgets/boundary tokens still pass. | `moon info` plus `pkg.generated.mbti` diff review, then `node scripts/validate-api-surface.mjs` |
| Renderer sync | Capability code, tests, docs, and visible Showcase coverage agree for the native Skia plus WebGPU mainline. Native WGPU diagnostics remain explicit and non-blocking. | `moon test moui/render --target native`, `moon test moui/render/skia --target native`, `moon test moui/render/webgpu_adapter --target wasm-gc`, Showcase Web build; optional `sh scripts/conformance-check.sh --render --wgpu-experimental` |
...
| Native Skia paragraph/bidi proof | macOS, Windows, and Linux native Skia renderer-proof manifests pass with real SkParagraph evidence for paragraph wrapping, bidi visual order, selection rectangles, and hit testing. Fallback geometry, caret-only diagnostics, heuristic visual-order logs, package tests, provider preflights, and local manifests without GitHub Actions provenance do not satisfy this gate. | `MOUI_SKIA_ENABLE_SKPARAGRAPH=1 MOUI_SKIA_REQUIRE_SKPARAGRAPH=1 moon run moui/tests/skia_text_emoji_smoke/native --target native` on each matching host, then `node scripts/validate-renderer-proof-manifest.mjs artifacts/conformance/renderer-proof/skia-native-<platform>.json --require-passed` for macOS, Windows, and Linux |
...
real-Skia Showcase first-frame evidence refreshed on 2026-06-03, and
Skia image lifecycle callback and host bridge evidence refreshed on 2026-06-04,
plus Web route/history bridge evidence refreshed on 2026-06-05, current-head
MoUI CI Web/renderer-proof evidence refreshed on 2026-06-09, and macOS-only
platform runtime evidence refreshed on 2026-06-09. The native SkParagraph
binding, renderer route, proof-token gate, and smoke-helper handoff were
refreshed on 2026-06-08; macOS, Windows, and Linux now have GitHub Actions
native Skia renderer-proof provenance in the latest all-green `MoUI CI` run,
while macOS platform-runtime readiness remains the separate manual
workflow-dispatch evidence described below.
Refresh the full gate set before a release candidate handoff.

| Gate | Current evidence | Status |
| --- | --- | --- |
| Daily baseline | The default `sh scripts/dev-check.sh` now covers local dependency guards, guidance consistency, API surface guardrails, renderer/provider and native Skia entrypoint wiring, checked conformance artifacts, dedicated capture/Web/platform/renderer-proof manifest validators, core/views/render/backend/package tests, and Showcase plus Markdown Editor app/Web wasm-gc validation. `sh scripts/dev-check.sh --theme-diagnostics` keeps Design Systems and `moui_theme` as addon diagnostic coverage, and `sh scripts/dev-check.sh --platform-examples-test` remains the current-host backend/provider extension; on Darwin it previously covered macOS backend/provider checks. On 2026-06-03, `scripts/macos-skia-renderer-smoke.sh --run-showcase-smoke` passed on Darwin with the pinned JetBrains Skia provider, covering MoUI renderer pixels plus first-frame Showcase macOS Skia presentation. | current default gate plus opt-in diagnostics and Darwin host extensions |
...
Platform evidence guard refresh: `scripts/validate-platform-evidence-manifest.mjs`
rejects `README.md` placeholder artifacts when a platform or native Skia
evidence block is marked `passed`, including nested `evidenceProvenance`
artifact lists. Passed runtime claims must point at concrete logs, manifests,
screenshots, or uploaded CI artifacts rather than scaffold documentation.

Skia async-image provider refresh: `render/skia` exposes
`skia_image_load_completion` for Skia encoded-image source decode completion
payloads, and macOS/Windows/Linux Skia providers install provider-owned
`HostAsyncImageLoader` hooks around that helper. Provider-created Skia renderers
now opt into post-present async image loading, and the real-Skia smoke records
`MoUI Skia async image second-frame smoke passed` when data URI and local-file
completions route through `HostImageResourceCompletionSource`, request repaint,
and draw on a second frame. Focused Skia renderer and provider tests prove
completion scheduling, lifecycle apply, and redraw routing through the host
scheduler; this remains provider/package plus macOS smoke evidence, not
matching-host off-main runtime proof across all native hosts.
...
| Gradient | Visual styling and sparkline/card brushes. | Showcase app tests assert `FillRoundedRectBrush` with `LinearGradient` and `RadialGradient`; renderer tests now guard WGPU/Web radial rounded/path brush payload preservation and shader selection, while Skia has renderer-local radial brush pixels. | Visible and command-level coverage exists, but WGPU/Web readiness still waits for renderer-proof CI radial center/mid/edge pixel artifacts. |
...
| Path/vector | Theme/renderer section includes a vector path card that emits filled and stroked `DrawPath` commands. | Renderer tests cover `PathSpec` tessellation including radial brush payload preservation, native draw-plan path items, Web host-call forwarding, fallback planning that keeps visible `DrawPath` out of fallback diagnostics, WGPU/Web path shader guards, and Skia renderer-local solid/linear-gradient/radial-gradient path pixels; Showcase app tests assert `DrawPath` emission. | Covered by visible Showcase demo plus renderer/Web adapter command-level evidence; Skia renderer-local pixels and WGPU/Web shader guards are renderer evidence, not platform runtime evidence. |
...
| Text shaping | Capability card lists follow-up status first; text/media section exercises text views. | Text conformance tests are primary evidence; Skia text now resolves `FontSpec` family, weight, style, representative coverage characters, and inferred BCP47 script language tags through Skia `FontMgr` `FontFallbackRequest`/`Font`, splits mixed-script text into grapheme-safe fallback segments for per-run FontMgr resolution, returns Skia font-metric baseline/height plus shaped-run cluster carets when SkShaper is linked or measured prefix carets otherwise, stabilizes representative combining-mark/Indic-matra/Arabic-mark/Thai-mark/Lao-mark/Sinhala-mark/Khmer-vowel-coeng/Myanmar-mark/Hangul-Jamo/keycap/emoji-modifier/VS/ZWJ/regional-indicator-pair/emoji-tag/prepend-mark cluster interiors in both caret paths, exposes `TextSystem::layout_paragraph()` line metrics, paragraph caret rectangles, selection rectangles, and hit-test geometry through the active measurement path, routes native paragraph layout through optional SkParagraph when the binding is built with `MOUI_SKIA_ENABLE_SKPARAGRAPH=1`, retries emoji font candidates on the system FontMgr path, clips aligned glyph drawing to `TextRun.frame`, and renders through optional SkShaper shaped glyph runs when linked or positioned glyph runs otherwise. The native diagnostic matrix now also injects `skia_text_system()` into a public `AppRuntime` text field and asserts that composition caret geometry and selection highlight drawing consume the Skia measurement path; this is app-runtime geometry evidence, not native-platform IME runtime evidence. The local `moui_skia` text descriptor update is consumed by `skia_text_descriptor_preflight`, which audits fallback, measurement, shaping, shaped-run, and shaped-glyph resource plans in fallback-safe tests; this is cache/resource-key evidence, not typography parity evidence. `backend_info()` now also includes a fallback-safe `text maturity audit partial` summary that counts the audited Skia baseline, missing-glyph recovery rule, mixed-run fallback, and Unicode 17 grapheme boundary contract separately from tracked bidi, paragraph line-breaking, and color emoji gaps. macOS Skia provider defaults now match Windows/Linux by selecting `SystemFontMgr` for normal Showcase, Markdown Editor, and Mo Workbench entrypoints, while macOS first-frame smoke entrypoints explicitly switch to `EmptyTypeface` only when their exit-after-first-present flag is set. Default renderer smoke continues to cover Skia FontMgr/SkShaper evidence plus bounded `TextRun.frame` clipping; provider validation rejects non-empty run-layout carets that do not cover the input, and Cosmic run-layout tests assert glyph output plus monotonic caret coverage through the provider-safe mapped layout path for representative emoji clusters. Showcase app tests assert the follow-up row is visible. | Do not use Showcase labels, Skia descriptor resource plans, the text maturity audit, basic Skia font matching/metrics/paragraph geometry, package-only tests, provider preflights, fallback geometry, Unicode 17 grapheme segmentation, or heuristic visual-order logs as proof of native SkParagraph/bidi/color-emoji/typography parity. Native SkParagraph/bidi readiness requires passed macOS, Windows, and Linux renderer-proof manifests. |
| Emoji text | Capability card lists follow-up status first. | Diagnostic text conformance covers single-codepoint, variation-selector, and ZWJ emoji measurement/caret invariants; renderer tests now cover native RGBA color glyph payload parsing/upload, text vertex shader marking, Web color glyph runtime events with font/glyph metadata, Cosmic platform emoji fallback loading/resolution, provider-safe Cosmic run-layout caret coverage, Cosmic color swash preservation, CoreText AppleColorEmoji format selection, Skia system-FontMgr `FontFallbackRequest` matching that prefers emoji coverage characters and inferred language tags, Skia text/emoji smoke metadata for the emoji font/glyph path including a source/script/fallback-request-character-aware stable glyph key, measured glyph dimensions, fallback script/language-tag metadata, request language-count matching, fallback request character metadata, and missing-glyph recovery metadata, and fallback caret stabilization for representative combining-mark/Indic-matra/Arabic-mark/Thai-mark/Lao-mark/Sinhala-mark/Khmer-vowel-coeng/Myanmar-mark/keycap/emoji-modifier/VS/ZWJ/regional-indicator-pair/emoji-tag/prepend-mark cluster interiors. Showcase app tests assert the partial follow-up row is visible. | Keep native/Web/Skia `partial`: the evidence does not prove full native emoji font fallback across all providers, ZWJ/color emoji conformance, browser rasterization determinism, exact typeface/glyph-id determinism, or provider shaping parity. |
| Async image | Capability card lists follow-up status first; image demos render ordinary images. | Native/Web renderer tests expose image resource records plus revisioned snapshots; native WGPU and Skia provider packages now forward snapshots through `HostWindowRenderer`; `moui/render` tests cover `ImageResourceLoadCompletion` ready/failed application on the shared lifecycle, and `moui/backend/host` tests cover per-window revision routing, redraw de-duplication, previous/current repaint result counts plus tracked-window revision/status-count snapshots for loading/ready/failed/disposed records, revision high-water handling that ignores stale lower snapshots, closed-window discard/removal with dropped current counts, the `HostWindowRenderer` image-resource callback setter, `HostWindowRenderer::apply_image_resource_load_completion` forwarding/default behavior, `HostImageResourceCompletionSource` publish/complete/redraw/stale-revision/closed-window completion routing diagnostics, `HostAsyncImageLoader` loading-record scheduling, in-flight de-duplication, cancellation cleanup, and late completion gating, plus `HostNativeAsyncImageSource` deferred request capture, later completion delivery, cancellation cleanup, and missing late-completion diagnostics. WGPU tests cover `native_image_load_completion` ready/failed payloads for native source decode, and macOS/Windows/Linux WGPU provider tests pin provider-owned loader scheduling through the host completion source; macOS, Windows, and Linux hosts now use the tracker around their redraw loops, install baseline-guarded renderer callbacks, invoke optional provider-owned async image loader hooks after present baseline, and cancel in-flight image loads during window disposal; Skia provider-created renderers opt into post-present async image loading, Skia providers forward callback setters to `SkiaRasterRenderer`, the real-Skia smoke proves data URI/local-file completions can request repaint and draw on a second frame, Skia caches immutable failed image sources with diagnostics before placeholder drawing, retries local-file failures when the file later appears, records disposed cached image resources during renderer disposal, and notifies optional renderer-local callbacks with revisioned snapshots for ready/disposed transitions plus unchanged failed-cache de-duplication; backend Web tests cover app/host-visible `WebRenderer::image_resources` and `WebRenderer::image_resource_snapshot`; Web records submitted sources as loading, refreshes ready/failed records from the browser image cache, and the canonical Web boot path schedules a redraw after browser image load/error notifications. | Still partial until off-main provider/platform async loader implementations and matching-host runtime artifacts prove real native loader completions on each host; Web no longer depends on a manual app action to observe browser image completion. |
...
2. Async image diagnostics
   - Current status: renderer-neutral lifecycle records model loading, ready,
     ...
     apply port;
     `HostAsyncImageLoader` scans loading records, de-duplicates in-flight
     `(window, source)` work, and gates cancelled or late completion callbacks
     before they can apply renderer state;
     native WGPU and Skia provider packages forward renderer snapshots through
     `HostWindowRenderer`, WGPU providers install provider-owned loaders around
     `native_image_load_completion` for PNG/JPEG/BMP source decode completions,
     Skia providers forward image-resource callback setters, and native hosts
     install baseline-guarded callbacks plus optional provider-owned loader
     hooks after present baseline; the host image repaint
   ...
   - Done when: both host source tests and native provider/platform evidence are
     ...
     implementations into the host scheduler and renderer completion/apply
     port, matching hosts must record redraw artifacts for late completions, or
     the missing pieces must be explicitly scoped out of the preview handoff.
     ...
     helpers, host loading-record scheduling, native host provider hook wiring,
     WGPU provider-owned source completion wiring, and host completion-source
     routing are current, but they do not yet prove that every native renderer
     has a real off-main async loader.
...
3. Emoji and text shaping evidence
   - Current status: Web can rely on browser font rasterization. Native WGPU can
     ...
     the provider-safe mapped layout path. Core fallback tests now use the
     shared UAX-style grapheme boundary scanner for CRLF/control segmentation,
     Extend/ZWJ/SpacingMark, Prepend, regional-indicator pairs, emoji ZWJ
     restrictions, emoji tags, Hangul L/V/T sequences, and Indic virama/linker
     conjuncts while preserving the per-character caret array; basic text
     caret movement, selection, delete ranges, rich-text hit testing, raw UTF-8
     offset conversion, and every-index IME `nearest_boundary_utf8_offset`
     conversion share those boundaries. Skia tests now cover the same
     ...
     on the system FontMgr path before default-font fallback, require
     new Skia-native color emoji proof recordings to carry fallback
     script/language tag-list/count metadata plus fallback request character and
     missing-glyph recovery metadata,
     require missing-glyph fallback to be complete or reduce missing glyph ids, keep
     mixed-run fallback font segments from splitting Indic virama plus Extend
     plus consonant clusters, inject the Skia
...
1. Render inspector diagnostics
   - Current status: `RenderInspectorSnapshot` can be built from an
     ...
     mouse, and click fallback routing plus the browser route/history bridge:
     pointer-capable browsers ignore compatibility mouse/click activation
     entirely, non-pointer fallback mode still de-duplicates delayed fallback
     events whose rounded coordinates drift from the original pointer event,
     and route tests cover initial route normalization,
     `pushState`/`replaceState`, `back`/`forward`, `popstate`, and wasm
     `web_dispatch_route` export wiring. This keeps delayed app rebuilds from
     replaying a single browser click as multiple MoUI pointer activations and
     keeps Web route subscriptions host-driven. This remains static/HTTP
     handoff evidence rather than browser WebGPU presentation proof.
     The shared
