...
| P9 | App production dependencies must not include `moui/runtime`, `moui/backend/*`, `moui/render/*`, platform objects, renderer providers, or `moui_theme/*` (unless a design-system preview). Showcase diagnostics belong in a sibling integration package. | `validate-api-surface.mjs` + `validate-maintenance-baseline.mjs` | none |
