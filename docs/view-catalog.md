...
| --- | --- | --- | --- | --- |
| Forms | ready with app-owned validation | `form`, `form_section`, `form_field`, `form_field_state`, `form_validation_summary`, `form_actions`, `form_workflow_bar`, `form_error`, `form_helper_text`, `input_group`, `clearable_text_field`, `password_field`, `number_field`, `stepper`, `multiline_text_area_shell`, plus `text_field`, `checkbox`, `toggle`, `segmented_control`, `picker`, and `datepicker` | Showcase Forms and Controls; Settings example | Validation rules, async validation, and actual focus changes stay in the app/runtime model; reusable view helpers now cover first-invalid focus targets, submit guards, and Enter/Escape action status. |
| Navigation | ready for controlled shells | `navigation_stack`, `router_stack`, `route_header`, `section_nav`, `sidebar`, `breadcrumb`, `split_view`, `master_detail`, `wizard`, `tab_view` | Showcase Navigation Shell; Showcase router/state tests | Deep-link integration, browser history, animated route transitions, focus trapping across presentation surfaces, and resizable split panes remain app/host follow-up work. |
| Dialogs and menus | partial | `dialog`, `alert`, `dialog_host`, `sheet_host`, `sheet`, `popover`, `dropdown`, `combobox`, `autocomplete`, `menu_button`, `menu_bar`, `command_menu`, `context_menu_region`, `command_palette` | Showcase Navigation Shell and Interaction Lab; Command Palette example | These are TEA-controlled view-level surfaces. Native context menus, global menu-bar integration, focus trapping, and host-modal semantics remain host/framework follow-up work. |
| Data views | ready with app-owned data logic | `data_filter_bar`, `data_filter`, `selection_toolbar`, `table`, `table_column`, `column_visibility_panel`, `selectable_list`, `pagination`, `detail_panel`, `tree_view`, `description_list`, `avatar`, `avatar_group`, `virtual_list`, `sectioned_list`, `scroll_to_index`, `lazy_list`, `lazy_grid` | Showcase Data and Layout; upgraded Data Table example | Filtering rules, sorting rules, selected-row/list-item sets, visible-column sets, column resizing/reordering, and row virtualization policy stay in app packages today. |
| Feedback and workflow states | ready for app-owned state | `toast`, `toast_stack`, `snackbar`, `banner`, `callout`, `progress_status`, `inline_error`, `empty_state`, `loading_state`, `error_state`, `status_badge`, `badge`, `stat_card`, `accordion`, `disclosure`, `collapsible_panel`, `drop_zone`, `file_import_panel` | Showcase Feedback and Interaction Lab; File Importer example | Toast queue storage, timers, host notifications, progress task lifecycle, and native file-picker UX remain app/host-owned. |
| Input, gestures, and accessibility | partial | `shortcut_button`, `focus_ring`, `FocusScope` helpers, `ActionCommand`, keyboard shortcut dispatch, `View::on_long_press`, `View::on_double_tap`, `View::on_drag`, semantics roles and state modifiers including disabled, pressed, selected, checked, expanded, invalid, and required | Showcase Interaction Lab shows focus order, first-invalid targeting, Enter/Escape command targets, shortcuts, visible app-owned focus rings, semantic states, gestures, and file drop; Navigation Shell shows command metadata; focused core/views tests cover dispatch and semantics | Runtime focus traversal exists, but focus trapping, IME parity, accessibility state announcements, and full platform accessibility adapters remain follow-up work. |
...
| `focus_ring` | `views/controls.mbt` | Border/theme | Focusable group with selected state | `views/views_test.mbt` | Showcase Interaction Lab | Visual wrapper for app-owned focus ids. It exposes a focus action plus selected/unselected semantics without changing runtime focus traversal or host focus trapping. |
...
| Constructor | Source | Theme | Semantics | Tests | Example coverage | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| `toast` / `toast_stack` / `snackbar` | `views/feedback.mbt` | Tone colors/surface | Group with optional dismiss action | `views/views_test.mbt` | Showcase Feedback | App-owned transient/status surfaces. `toast_stack` renders an app-owned notification queue with optional per-item action and dismiss messages; timers and queue storage stay in the app model. |
...
| `drop_zone` / `file_import_panel` | `views/file_import.mbt` | Surface/button/text | Button-like drop target/group | `views/views_test.mbt` | Showcase Interaction Lab | Drag/drop maps through `View::on_file_drop`; browse buttons emit app messages so effect-capable apps can call `HostAppServices::open_file` from `Effect::dispatch` and register `HostAppServices::on_completed` for pending responses without making `views` depend on backend packages. Web hosts may surface browser file names or handles while native hosts can surface filesystem paths. |
...
| `data_filter_bar` / `data_filter` | `views/data_display.mbt` | Search field/chips/ButtonStyle | Group with selected/disabled filter chips | `views/views_test.mbt` | Showcase Data, Data Table | Controlled search/filter toolbar for table and list surfaces. Query, selected filters, result count, and clear policy remain app-owned. |
| `selection_toolbar` | `views/data_display.mbt` | ButtonStyle/text/surface | Group with selected state | `views/views_test.mbt` | Showcase Data, Data Table | Controlled selected-row summary with bulk action buttons and optional clear action. The app owns the selected row ids and action effects. |
| `table` / `table_column` | `views/data_display.mbt` | Surface/text/border/ButtonStyle | Grid with selectable row items and optional sortable header buttons | `views/views_test.mbt` | Showcase Data, Data Table | Controlled text table with optional custom cell view, selected row, row-select messages, sortable header state, and empty state; sorting rules, filtering, and resizing stay app-owned. |
| `column_visibility_panel` | `views/data_display.mbt` | Checkbox/text/surface | Group with selected and disabled column items | `views/views_test.mbt` | Showcase Data, Data Table | Controlled column visibility chooser for table workflows. Apps own the visible-column set and can lock required columns while column resizing and reordering remain follow-up work. |
...
| `pagination` | `views/data_display.mbt` | ButtonStyle/text | Group with disabled edge buttons | `views/views_test.mbt` | Showcase Data, Data Table | Controlled pagination bar; page indexing and data slicing stay app-owned while previous/next buttons disable at the first and last page. |
