...
| Public API audit | `moon info` 与 `validate-api-surface.mjs` 审查根门面 TEA API、runtime-owned AppBuilder / PlatformEntry / AppLaunchRequest、中立 SurfaceContext 与图片契约、renderer factories、host diagnostics 及平台 options。根 `wzzc-dev/moui` 不依赖 runtime，也不暴露 `run_app`；具体 renderer 构造和 decode API 只存在于 `moui/render`。 | 当前 |
...
| Renderer sync | render/capabilities.mbt、renderer package tests 和 docs/renderer-capability-report.md 仍是事实来源。应用入口注册有序 RendererFactory；平台 backend 创建 SurfaceContext，且只接收中立 WindowRenderer。平台 HostImageSource 读取原始字节，Skia、Sun 与 WGPU 各自拥有 RendererImageDecoder 解码/completion 逻辑。AsyncImageLoader 和 ImageResourceRepaintTracker 让 completion、cancellation、revision 与逐窗口 repaint 路由保持 renderer-neutral。 | 当前 |
...
`ImageResourceCompletionSource` 路由、请求 repaint 并在第二帧绘制时记录 `MoUI Skia async image second-frame smoke passed`。
...
Deferred native async-image source refresh：`backend` 现在暴露
`NativeAsyncImageSource`，使 platform loaders 可以记录 pending native image
requests，并稍后通过相同的 `AsyncImageLoader` 和 repaint route 交付 ready/failed completions。Host tests 证明 schedule-return-first
