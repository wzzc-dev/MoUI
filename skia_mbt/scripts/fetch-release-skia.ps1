param(
  ...
  [string] $LinkMode = $(if ($env:SKIA_MBT_SKIA_LINK_MODE) { $env:SKIA_MBT_SKIA_LINK_MODE } else { "static" }),
...
if ($PrintEnv) {
  @(
    "SKIA_MBT_PROVIDER=release"
    "SKIA_MBT_SKIA_PROVIDER=release"
    "SKIA_MBT_RELEASE_OWNER=$($provider.owner)"
    "SKIA_MBT_RELEASE_REPO=$($provider.repo)"
    "SKIA_MBT_RELEASE_TAG=$Tag"
    "SKIA_MBT_RELEASE_URL=$($provider.release_url)"
    ...
    "SKIA_MBT_SKIA_LINK_MODE=$resolvedLinkMode"
