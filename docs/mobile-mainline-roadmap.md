...
| Mobile runtime smoke | Recorder writes optional `renderer` (+ pending `gpuPromotionEvidence` skeleton when `gpuPromoted=true`, `claimed=false`) | iOS Simulator CG **`passed`** (Metal); Android CG **`partial`** (`vulkan-gpu` + `gpuAvailable=true`, NDK28 libc++); HarmonyOS HVD CG **`partial`** (`egl-gpu` + `gpuAvailable=true`). Emulator install/verify: `docs/android-support.md` / `docs/harmonyos-support.md`. |
