# MoUI Platform Showcase Android

This is an experimental Android shell for the Platform Showcase example. The Java
`Activity` owns the `SurfaceView` lifecycle, JNI owns `ANativeWindow` handle
acquisition, and the MoonBit package `examples/platform_showcase/android` owns
the embedded MoUI runtime session.
...
Use `scripts/build-platform-showcase-android-apk.sh --fallback-skia` only for a fast
