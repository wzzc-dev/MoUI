...
link_flags="$skia_library_link_flags -lc++ -framework CoreFoundation -framework CoreGraphics -framework CoreText -framework ImageIO -framework ApplicationServices -framework UniformTypeIdentifiers"
...
restore_native_pkg() {
  ...
  if [[ -f "$renderer_pkg_backup" ]]; then
    ...
    echo "Restored moui/tests/skia_renderer_smoke/native/moon.pkg after macOS Skia smoke."
  fi
  if [[ -f "$text_emoji_pkg_backup" ]]; then
    cp "$text_emoji_pkg_backup" "$text_emoji_pkg"
    rm -f "$text_emoji_pkg_backup"
    echo "Restored moui/tests/skia_text_emoji_smoke/native/moon.pkg after macOS Skia smoke."
  fi
}
...
EOF
  echo "Wrote temporary moui/tests/skia_renderer_smoke/native/moon.pkg with macOS Skia link flags."
...
EOF
  echo "Wrote temporary moui/tests/skia_text_emoji_smoke/native/moon.pkg with macOS Skia link flags."
