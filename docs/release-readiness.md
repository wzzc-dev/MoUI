...
2. Async image diagnostics
   - Current status: renderer-neutral lifecycle records model loading, ready,
     ...
     `HostWindowRenderer`; Skia caches failed sources with diagnostics before
     placeholder drawing, clears cached images and records disposed resources
     during renderer disposal, and the backend WebRenderer facade forwards Web
     snapshots to app/host integration code.
