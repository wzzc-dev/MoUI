...
2. Async image diagnostics
   ...
   - Done when: both host source tests and native provider/platform evidence are
     ...
     helpers, host loading-record scheduling, and host completion-source routing
     are current, but they do not yet prove that every native renderer has a real
     out-of-render async loader.
