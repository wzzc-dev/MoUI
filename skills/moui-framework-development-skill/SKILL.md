...
- `Program`, `Effect[Msg]`, and `Subscription[Msg]` are the default app model
  ...
  `Subscription::plan_summary`, subscription lifecycle/plan counters, and
  ignored stale-dispatch counters for callbacks captured by canceled or
  destroyed subscription lifetimes; effect runners and subscription adapters
  still own any concrete async work outside `core`.
