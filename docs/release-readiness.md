...
| Devtool counters | Core inspector snapshots expose runtime, layout, semantics, frame, render command, rebuild/layout/paint/draw-command pass counters, TEA dispatch/update/message-queue/effect-plan/effect-kind counters that distinguish send, anonymous dispatch, and structured run, latest effect summaries with structured effect descriptors, duplicate effect descriptor-key counters/names, subscription-plan counters/summaries with planned source descriptors and duplicate key names, active subscription descriptors, app subscription lifecycle entries, and ignored stale subscription dispatch counters; render snapshots also report open clip/layer/filter scopes and unbalanced pops, and Showcase Diagnostics surfaces render command/scope counters plus TEA message, structured effect, subscription plan, active subscription, and descriptor labels with app-test coverage. | ready for command-level diagnostics |
