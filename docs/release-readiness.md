...
| Public view model | `views/` constructors are documented as returning opaque `@core.View[Msg]`; `Program::simple`, `Program::new`, `Effect[Msg]`, and app-level `Subscription[Msg]` are the app-facing TEA surface; public API edits require `moon info`. `backend/host` now exposes `HostAppServices::completion_subscription` for app-owned pending host-service completions while concrete timer, window, and route subscription adapters remain follow-up work outside `core`. | ready with adapter follow-ups |
