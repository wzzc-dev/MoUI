...
| `route_header` | `views/navigation.mbt` | Surface/text/ButtonStyle | Group | `views/views_test.mbt`, Showcase tests | Showcase Navigation Shell | Controlled page header for app-owned route title, subtitle, and action buttons; routing, deep links, and history stay outside the view helper. |
| `section_nav` | `views/navigation.mbt` | ButtonStyle/text/surface | List with selected/disabled list items | `views/views_test.mbt`, Showcase tests | Showcase Navigation Shell | Controlled section switcher built from `SectionNavItem` descriptors. Apps own the selected id, routing effects, history, and unavailable-section policy. |
