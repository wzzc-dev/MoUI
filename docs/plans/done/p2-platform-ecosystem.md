...
- [x] Add versioning/deprecation/compatibility policy, third-party control and host-service extension templates, quality metadata, and an external-consumer compatibility gate.
- [x] Run focused checks and required static/API/closure/guidance validators after each public-surface or package-layout commit; native package tests were attempted but blocked by the environment's unavailable Skia download.
- [x] Commit each completed part separately.
...
| 2026-08-23 | Added SemVer/deprecation policy, component quality metadata, control/host-service extension templates, metadata validation, and CI enforcement for external consumers. Script, maintenance, guidance, and reference checks pass. |
