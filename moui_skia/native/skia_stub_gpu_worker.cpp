...
bool render_vulkan_picture(
  ...
  if (!picture) {
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_WARN, "MoUIWindow", "vulkan present fail=no-picture");
#endif
    return false;
  }
  ...
  if (acquire_result == VulkanAcquireResult::Recreate) {
    ...
    if (!ensure_vulkan_swap_chain(worker)) {
#if defined(__ANDROID__)
      __android_log_print(ANDROID_LOG_WARN, "MoUIWindow", "vulkan present fail=swap-recreate");
#endif
      return false;
    }
...
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_WARN, "MoUIWindow", "vulkan present fail=submit");
#endif
    return false;
  }
  VkFence image_fence = worker->vulkan_image_fences[image_index];
  if (vkResetFences(worker->vulkan_device, 1, &image_fence) != VK_SUCCESS) {
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_WARN, "MoUIWindow", "vulkan present fail=reset-fence");
#endif
    return false;
  }
...
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_WARN, "MoUIWindow", "vulkan present fail=queue-submit");
#endif
    return false;
  }
