...
usage() {
  cat <<'EOF'
Usage: scripts/linux-build-skia.sh [options]
...
gn_args='is_official_build=true is_debug=false cc="clang" cxx="clang++" skia_use_gl=false skia_use_vulkan=false skia_use_metal=false skia_use_dawn=false skia_use_x11=false skia_enable_gpu=false skia_enable_pdf=false skia_enable_svg=false skia_enable_skshaper=false skia_enable_skparagraph=false skia_enable_tools=false skia_use_system_expat=false skia_use_system_freetype2=false skia_use_system_harfbuzz=false skia_use_system_icu=false skia_use_system_libjpeg_turbo=false skia_use_system_libpng=false skia_use_system_libwebp=false skia_use_system_zlib=false extra_cflags=["-DSK_DISABLE_LEGACY_PNG_WRITEBUFFER"]'
...
if [[ $dry_run_config -eq 1 ]]; then
  cat <<EOF
Linux Skia source build dry run:
  work_dir=$work_dir
...
echo "Linux Skia source build environment:"
...
if [[ ! -f "$out_dir/libskia.a" && ! -f "$out_dir/libskia.so" ]]; then
  echo "Skia build completed, but no libskia.a or libskia.so was found in $out_dir" >&2
  exit 1
fi
if [[ $enable_skparagraph -eq 1 ]]; then
  for paragraph_lib in skparagraph skshaper skunicode_core skunicode_icu; do
    if [[ ! -f "$out_dir/lib$paragraph_lib.a" && ! -f "$out_dir/lib$paragraph_lib.so" ]]; then
      echo "Skia build completed, but no lib$paragraph_lib.a or lib$paragraph_lib.so was found in $out_dir" >&2
      exit 1
    fi
  done
fi
...
Run smoke test:
  bash scripts/linux-skia-smoke.sh --skia-include "$skia_dir" --skia-lib-dir "$out_dir" --extra-link-flags "-lfontconfig -lfreetype -lharfbuzz -lpthread -ldl -lm"$([[ $enable_skparagraph -eq 1 ]] && printf ' --enable-skparagraph')
EOF
