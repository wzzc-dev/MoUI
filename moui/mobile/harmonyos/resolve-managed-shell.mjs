...
if (app.harmonyos.shellMode !== "managed") {
  throw new Error(
    `app ${options.app} requests HarmonyOS shellMode=${app.harmonyos.shellMode}; managed shell required`,
  );
}
if (app.schemaVersion !== 2 || app.shellApiVersion !== 1 || app.runtimeAbiVersion !== 1) {
  throw new Error("managed HarmonyOS shell requires schema v2, shell API v1, and runtime ABI v1");
}
...
if (outputRoot === parse(outputRoot).root || outputRoot === workspaceRoot || outputRoot === mouiRoot ||
...
const generatedConfig = [
  ...
  `  fullscreen: ${app.mobile.systemUi.fullscreen ? "true" : "false"},`,
  `  statusBar: ${JSON.stringify(app.mobile.systemUi.statusBar)},`,
  `  orientation: ${JSON.stringify(app.mobile.orientation)}`,
  "};",
  "",
].join("\n");
writeFileSync(
...
for (const [index, resource] of app.mobile.resources.entries()) {
...
writeJson(resolve(outputRoot, ".moui-managed-shell.json"), {
  ...
  shellMode: app.harmonyos.shellMode,
