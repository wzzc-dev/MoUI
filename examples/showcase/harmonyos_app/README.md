# MoUI Showcase HarmonyOS

> Release N legacy fixture: normal MoUI Showcase builds now stage the
...
experimental MoUI Showcase HarmonyOS route. The shell uses the reusable
ArkTS, NAPI, and CMake support from `moui/mobile/harmonyos`; app metadata and
native export names come from `examples/showcase/mobile.json`.

Existing Component Gallery captures are historical and do not establish
runtime evidence for the renamed MoUI Showcase application. Fresh non-fallback
first-frame, input, resize, lifecycle, and platform-service evidence is pending.
See
...
```sh
scripts/build-showcase-harmonyos-hap.sh \
...
MOUI_SKIA_DISABLE_PREBUILD_SKIA=1 \
  moon check examples/showcase/harmonyos_skia --target native
...
```text
artifacts/harmonyos/showcase/MoUIShowcase.hap
```
...
## 7. Install And Launch MoUI Showcase

Install the non-fallback HAP and start its entry ability:

```sh
HDC="$HARMONYOS_SDK_HOME/toolchains/hdc"
HAP=artifacts/harmonyos/showcase/MoUIShowcase.hap

"$HDC" install -r "$HAP"
"$HDC" shell aa start \
  -a EntryAbility \
  -b dev.wzzc.moui.showcase \
  -m entry
```

The expected first screen is the MoUI Showcase rendered inside the
...
```sh
HDC="$HARMONYOS_SDK_HOME/toolchains/hdc"
REMOTE_SCREENSHOT=/data/local/tmp/moui-showcase.jpeg
LOCAL_SCREENSHOT=artifacts/harmonyos/showcase/moui-showcase.jpeg
