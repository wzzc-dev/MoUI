...
| Gradient | Visual styling and sparkline/card brushes. | Showcase app tests assert `FillRoundedRectBrush` with `LinearGradient` and `RadialGradient`; renderer tests cover Skia radial brush pixels plus WGPU/Web deterministic radial rounded center-to-edge fallback and radial path center-color fallback. | Covered by visible demo plus command-level app test. |
