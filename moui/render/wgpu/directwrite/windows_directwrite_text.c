...
static int32_t moui_windows_parse_font_spec(
...
static int32_t moui_windows_parse_font_registration(
  moonbit_bytes_t bytes,
...
MOONBIT_FFI_EXPORT
int32_t moui_windows_directwrite_debug_font_spec_family_count(
  moonbit_bytes_t font_spec
) {
  MouiNativeFontSpec spec = {0};
  if (!moui_windows_parse_font_spec(font_spec, &spec)) {
    return -1;
  }
...
MOONBIT_FFI_EXPORT
int32_t moui_windows_directwrite_debug_font_registration_data_len(
  moonbit_bytes_t payload
) {
  MouiNativeFontRegistration registration = {0};
  if (!moui_windows_parse_font_registration(payload, &registration)) {
    return -1;
  }
...
MOONBIT_FFI_EXPORT
int32_t moui_windows_directwrite_debug_font_spec_protocol_version(void) {
  return MOUI_NATIVE_FONT_SPEC_VERSION;
}

MOONBIT_FFI_EXPORT
int32_t moui_windows_directwrite_debug_font_registration_protocol_version(void) {
...
MOONBIT_FFI_EXPORT
moonbit_bytes_t moui_windows_directwrite_measure_utf32(
  moonbit_bytes_t utf32,
  moonbit_bytes_t font_spec
) {
  (void)utf32;
  if (!moui_windows_parse_font_spec(font_spec, NULL)) {
...
MOONBIT_FFI_EXPORT
moonbit_bytes_t moui_windows_directwrite_layout_run_utf32(
  moonbit_bytes_t utf32,
  moonbit_bytes_t font_spec,
  double max_width,
  double scale_factor
...
MOONBIT_FFI_EXPORT
moonbit_bytes_t moui_windows_directwrite_raster_glyph(moonbit_bytes_t payload) {
  (void)payload;
  return moonbit_make_bytes(0, 0);
}

MOONBIT_FFI_EXPORT
int32_t moui_windows_directwrite_register_font_data(
  moonbit_bytes_t payload
) {
  return moui_windows_parse_font_registration(payload, NULL);
}
