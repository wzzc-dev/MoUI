...
| `lazy_list` | `views/views.mbt` | N/A | List output | `views/views_test.mbt` | Showcase | Windows visible rows from data. |
| `lazy_grid` | `views/views.mbt` | N/A | List/grid output | `views/views_test.mbt` | Showcase | Windows visible grid rows from data. |
| `virtual_list` / `sectioned_list` / `scroll_to_index` | `views/views.mbt`, `views/views.mbt` | N/A | List output | `views/views_test.mbt`, Showcase tests | Showcase Layout | Overscanned windowed lists, grouped section headers, empty states, and controlled offset intents built on existing scroll/list primitives. |
...
| `router_stack` | `views/views.mbt` | Child-based | Selected child semantics | `views/views_test.mbt`, `core/app_framework_wbtest.mbt` | N/A | Selects destinations from `@core.RouterState`, preserving query params and restoration snapshots while staying above the renderer. |
