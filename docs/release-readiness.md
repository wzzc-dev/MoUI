...
| Native Skia paragraph/bidi proof | `moui_skia` exposes `skia_paragraph_available()` plus the native `Paragraph` wrapper for UTF-8 layout, line metrics, selection boxes, and hit testing; `render/skia` uses it when `MOUI_SKIA_ENABLE_SKPARAGRAPH=1` is configured and keeps fallback readiness flags false otherwise. Renderer-proof validation now requires `engine=skparagraph native_paragraph_ready=true line-metrics later-line-pixels` for `paragraphWrapping`, `engine=skparagraph bidi_visual_order_ready=true visual-order` for `bidiLayout`, and `engine=skparagraph selection-rects line-range rect-geometry hit-test` for `selectionRects`. The macOS helper has `--enable-skparagraph` and `--require-skparagraph`; binding-level macOS/Linux/Windows smoke/configure helpers expose equivalent flags. | pending until macOS, Windows, and Linux real-Skia renderer-proof manifests pass with GitHub Actions provenance |
