...
| `drop_zone` / `file_import_panel` | `views/file_import.mbt` | Surface/button/text | Button-like drop target/group | `views/views_test.mbt` | Showcase Interaction Lab | Drag/drop 通过 `View::on_file_drop` 映射；browse button 发出 app 消息，使应用可以调用 `AppServices::files().open_file(...)`，并用 `ServiceTask::effect` 把 typed result 转为 `Msg`，而不让 `views` 依赖 backend 包或向 model 暴露 request id。Web host 可以暴露浏览器文件名或 handle，而 native host 可以暴露 filesystem path。 |
...
| `router_stack` | `views/views.mbt` | 基于 child | Selected child semantics | `views/views_test.mbt`、`views/navigation/navigation_routing_test.mbt` | N/A | 从 `@views.RouterState` 中选择 destination，保留 query param 和 restoration snapshot，同时保持在 renderer 之上。 |
...
| `dialog_host` | `views/navigation_presentation.mbt` | SurfaceStyle | Presented 时为 Dialog | `views/views_test.mbt`、Showcase 测试；DSH Desktop | 带 dim scrim 和居中 dialog 的 modal content stack wrapper。可选 `on_backdrop` 用 app message 消费背景点击；省略时 scrim 不发消息。Presented dialog 默认应用 view-level focus trap；只有在有意构建 non-modal preview surface 时才传入 `trap_focus=false`。 |
