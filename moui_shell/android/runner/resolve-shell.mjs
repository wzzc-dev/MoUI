...
export const resolveAndroidPermissionCapabilities = capabilities => {
  ...
  for (const capability of capabilities) {
    ...
    if (!mapped) {
      throw new Error(
        `managed Android shell does not support shell.permissions capability ` +
...
const renderManifest = ({ app, orientation, permissions }) => {
  ...
  return [
    ...
    "        <provider",
    '            android:name="dev.wzzc.moui.shell.MoUIClipboardProvider"',
    '            android:authorities="${applicationId}.moui.clipboard"',
    '            android:exported="false"',
    '            android:grantUriPermissions="true" />',
    "        <activity",
    '            android:name="dev.wzzc.moui.shell.MoUIActivity"',
    ...
    `                android:value="${app.shell.systemUi.fullscreen ? "true" : "false"}" />`,
    "            <meta-data",
    '                android:name="dev.wzzc.moui.STATUS_BAR"',
    `                android:value="${xml(app.shell.systemUi.statusBar)}" />`,
    "            <intent-filter>",
...
export const resolveAndroidManagedShell = ({ app, buildDir, workspaceRoot = app?.paths?.workspaceRoot }) => {
  if (!app || app.schemaVersion !== 1 || app.shellApiVersion !== 1 || app.embeddingApiVersion !== 1 || app.shell.profile !== "handheld") {
    throw new Error("managed Android shell requires schema v1, handheld profile, shell API v1, and embedding API v1");
  }
  if (!app.android) throw new Error(`app ${app.id} does not configure Android`);
  if (!["managed", "ejected"].includes(app.android.runnerMode)) {
    throw new Error(`app ${app.id} requests unsupported Android runnerMode=${app.android.runnerMode}`);
  }
  ...
  const orientation = orientationValues.get(app.shell.orientation);
  if (!orientation) throw new Error(`unsupported managed Android orientation: ${app.shell.orientation}`);
  if (!["auto", "visible", "hidden"].includes(app.shell.systemUi.statusBar)) {
    throw new Error(`unsupported managed Android status bar mode: ${app.shell.systemUi.statusBar}`);
  }
  ...
  const managed = app.android.runnerMode === "managed";
  const outputRoot = join(resolvedBuildDir, "android", `${app.android.runnerMode}-shell`);
  rmSync(outputRoot, { recursive: true, force: true });
  ensureDir(outputRoot);

  const permissions = managed
    ? resolveAndroidPermissionCapabilities(app.shell.permissions || [])
    : [];
  ...
  const result = {
    ...
    platform: "android",
    app: app.id,
    runnerMode: app.android.runnerMode,
    ...
    systemUi: {
      fullscreen: app.shell.systemUi.fullscreen,
      statusBar: app.shell.systemUi.statusBar,
    },
    permissionCapabilities: [...(app.shell.permissions || [])],
...
const usage = `Usage: moui_shell/android/resolve-managed-shell.mjs --app <id> --build-dir <dir> [options]
...
Options:
  ...
  --app-config <path>      App-owned schema v1 shell.json.
  --app <id>               Shell app id.
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.error(`[moui-shell-android-config] ${error.message}`);
