...
usage() {
  cat <<'USAGE'
Usage: moui_shell/scripts/build-ios-app-core.sh --app <id>

Internal builder used by Xcode native targets. Prefer
moui_shell/scripts/build-ios-app.sh.
USAGE
}

shell_root="${MOUI_EMBEDDING_SHELL_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
...
elif [ -d "$workspace_root/moui" ]; then
  moui_root="$workspace_root/moui"
else
  moui_root="$workspace_root/.mooncakes/wzzc-dev/moui"
fi
sdk_root="${MOUI_SHELL_PACKAGE_ROOT:-$(dirname "$moui_root")/moui_shell}"
...
app_config="${MOUI_EMBEDDING_APP_CONFIG:-}"
app="${MOUI_EMBEDDING_APP:-}"
...
if [ -z "$app" ]; then
  echo "--app or MOUI_EMBEDDING_APP is required" >&2
  exit 2
fi

sdk="${MOUI_EMBEDDING_SDK:-${SDK_NAME:-iphonesimulator}}"
case "$sdk" in
...
arch="${MOUI_EMBEDDING_ARCH:-${ARCHS:-arm64}}"
arch="${arch%% *}"
deployment_target="${MOUI_EMBEDDING_DEPLOYMENT_TARGET:-${IPHONEOS_DEPLOYMENT_TARGET:-15.0}}"
fallback_skia="${MOUI_EMBEDDING_FALLBACK_SKIA:-0}"
renderer="${MOUI_EMBEDDING_RENDERER:-auto}"
configuration="${CONFIGURATION:-Debug}"
shell_mode="${MOUI_EMBEDDING_IOS_SHELL:-managed}"
project_root="${MOUI_EMBEDDING_PROJECT_ROOT:-}"
build_dir="${MOUI_EMBEDDING_BUILD_DIR:-$workspace_root/artifacts/ios/$app}"
...
if [ "$shell_mode" = "ejected" ]; then
  if [ -z "$project_root" ]; then project_root="$(dirname "$shell_root")"; elif [ "${project_root#/}" = "$project_root" ]; then project_root="$workspace_root/$project_root"; fi
  if [ -z "$app_config" ] && [ -f "$project_root/shell.json" ]; then app_config="$project_root/shell.json"; fi
  shell_lock="$shell_root/.moui-shell.json"
  node "$sdk_root/scripts/validate-ejected-lock.mjs" \
    --lock "$shell_lock" --platform ios --moui-root "$moui_root" \
    --shell-root "$sdk_root" --project-root "$project_root" --app-config "$app_config"
fi
...
node "$sdk_root/scripts/prepare-native-build.mjs" "${prepare_args[@]}"
...
' "$1" "${2:-$build_dir/shell-build.json}"
}
...
output_app="${MOUI_EMBEDDING_OUTPUT_APP:-}"
...
managed_manifest=""
{
  ...
  resolved_swift="${MOUI_EMBEDDING_IOS_RESOLVED_SWIFT:-}"
  resolved_manifest="${MOUI_EMBEDDING_IOS_RESOLVED_MANIFEST:-}"
  if [ -n "$resolved_swift" ] || [ -n "$resolved_manifest" ]; then
    ...
    node "$sdk_root/ios/runner/resolve-shell.mjs" "${resolver_args[@]}"
  fi
...
swift_flags=(
  ...
  -target "$swift_target"
  -sdk "$sdk_path"
  -I "$shell_root/ios/embedder/bridge/include"
)
include_flags=(
  -I "$shell_root/include"
  -I "$shell_root/ios/embedder/include"
  -I "$shell_root/ios/embedder/bridge/include"
  -I "$moon_home/include"
  -I "$skia_root/native"
)
...
compiled_main_alias="moui_embedding_moonbit_generated_main"
...
compile_c "$shell_root/ios/moui_ios_compat.c" "$obj_dir/moui_ios_compat.o"
compile_cxx \
  "$shell_root/embedding/native/moui_embedding_api_v1.cpp" \
  "$obj_dir/moui_embedding_api_v1.o" \
  -DMOUI_EMBEDDING_API_APP_MAIN="$compiled_main_alias"
compile_objcxx "$shell_root/ios/embedder/bridge/MoUIMobileRuntimeBridge.mm" "$obj_dir/MoUIMobileRuntimeBridge.o"
  ...
  log "Compiling Swift package module MoUIShellShell"
  "$swiftc" "${swift_flags[@]}" \
    -parse-as-library \
    -whole-module-optimization \
    -module-name MoUIShellShell \
    -emit-module \
    -emit-module-path "$obj_dir/MoUIShellShell.swiftmodule" \
    -emit-object \
    "${shell_swift_sources[@]}" \
    -o "$obj_dir/MoUIShellShell.o"
  objects+=("$obj_dir/MoUIShellShell.o")
