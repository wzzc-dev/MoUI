...
#define MOUI_EMBEDDING_APP_ARG "moui-shell-android"
#endif
...
constexpr const char *kLogTag = "MoUIShell";
constexpr const char *kNativeBridgeClass =
    "dev/wzzc/moui/shell/MoUINativeBridge";
...
std::string consume_utf8_buffer(moui_embedding_utf8_buffer_v1 raw,
  ...
  if (value.status != MOUI_EMBEDDING_API_STATUS_OK_V1 ||
    ...
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "moui-shell ABI buffer failure operation=%s status=%d",
...
jstring java_string_from_buffer(JNIEnv *env, moui_embedding_utf8_buffer_v1 raw,
                                const char *operation) {
  OwnedUtf8Buffer buffer(raw);
  const auto &value = buffer.get();
  if (value.status != MOUI_EMBEDDING_API_STATUS_OK_V1 ||
  ...
  if (!utf16_from_utf8(value.data, value.length, &units)) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "moui-shell ABI returned invalid UTF-8 operation=%s",
                        operation);
    return env->NewStringUTF("");
  }
  const jchar empty = 0;
  ...
  std::call_once(g_initialize_once, [] {
    ...
    if (g_initialize_result != MOUI_EMBEDDING_API_RESULT_TRUE_V1) {
      __android_log_print(
          ...
          "moui-shell runtime initialization failed app=%s status=%d",
    ...
    __android_log_print(ANDROID_LOG_INFO, kLogTag,
                        "moui-shell runtime initialized app=%s "
    ...
    __android_log_print(
        renderer_configured == MOUI_EMBEDDING_API_RESULT_TRUE_V1
            ? ANDROID_LOG_INFO
            : ANDROID_LOG_WARN,
        kLogTag,
        "moui-shell renderer configure requested=%s status=%d detail=%s",
        MOUI_EMBEDDING_RENDERER_REQUESTED, renderer_configured,
        renderer_status.c_str());
  });
  return g_initialize_result == MOUI_EMBEDDING_API_RESULT_TRUE_V1;
}

jboolean boolean_result(const char *operation, int32_t result) {
  if (result < MOUI_EMBEDDING_API_STATUS_OK_V1) {
    __android_log_print(ANDROID_LOG_ERROR, kLogTag,
                        "moui-shell ABI call failed operation=%s status=%d",
                        operation, result);
  }
  return result == MOUI_EMBEDDING_API_RESULT_TRUE_V1 ? JNI_TRUE : JNI_FALSE;
}
...
jboolean native_attach_surface(JNIEnv *env, jclass, jobject surface, jint width,
  ...
  __android_log_print(ANDROID_LOG_INFO, kLogTag,
                      "moui-shell attach app=%s status=%d width=%d height=%d",
                      MOUI_EMBEDDING_APP_ID, result, width, height);
...
jboolean native_complete_clipboard(JNIEnv *env, jclass, jint session_generation,
  ...
  const moui_embedding_bytes_view_v1 byte_view = {
...
void native_detach_surface(JNIEnv *, jclass) {
  ...
  __android_log_print(ANDROID_LOG_INFO, kLogTag,
                      "moui-shell detach app=%s status=%d", MOUI_EMBEDDING_APP_ID,
                      result);
}
...
jboolean native_destroy_application(JNIEnv *, jclass) {
  ...
  __android_log_print(ANDROID_LOG_INFO, kLogTag,
                      "moui-shell application destroy app=%s status=%d",
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
  ...
  if (!moui_embedding_api_v1_is_compatible(runtime_api)) {
    __android_log_print(
        ANDROID_LOG_ERROR, kLogTag,
        "incompatible MoUI shell runtime ABI expected=%u actual=%u size=%u "
        "capabilities=%llu",
