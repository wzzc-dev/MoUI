...
| --- | --- | --- |
| Daily validation | `sh scripts/dev-check.sh` runs the bounded package tests, API surface guard, checked conformance artifact guard, dedicated checked-artifact validators for capture/Web/platform/renderer-proof manifests, and Web wasm-gc builds for Showcase, Markdown Editor, and `examples/design_systems/web_wasm`. | ready |
...
| Examples | Showcase, Markdown Editor, and Design Systems remain runnable docs; new user-facing framework features have visible Showcase coverage or a recorded reason to skip it, while design-system work stays isolated in the dedicated addon preview example. | App package tests plus Web wasm-gc builds |
