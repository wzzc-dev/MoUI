...
const generatedRegistry = plugins => {
  ...
  const installBody = installs.length > 0 ? [
    ...
    "    fun install(context: Context, capabilities: MoUIShellPluginCapabilities) = Unit",
  ];
  return [
...
export const prepareAndroidPlugins = ({ plugins = [], buildDir, runnerMode }) => {
  if (!isAbsolute(buildDir)) throw new Error("Android plugin buildDir must be absolute");
  if (!["managed", "ejected"].includes(runnerMode)) {
    throw new Error(`Android plugin shell mode must be managed or ejected: ${runnerMode}`);
  }
  ...
  const registryFile = join(
    generatedKotlinDir,
    "dev", "wzzc", "moui", "shell", "MoUIGeneratedPluginRegistry.kt",
  );
