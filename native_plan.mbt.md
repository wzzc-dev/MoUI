...
`skia_mbt native smoke test passed` marker, and checks that `native/moon.pkg`
was restored. The lower-level helper rewrites `native/moon.pkg` only for the
duration of the run, adding `SKIA_MBT_HAS_SKIA`, the supplied include directory,
...
_build/native/debug/build/skia_mbt_native_smoke.exe
```
...
`skia_mbt native smoke test passed`, so a successful Linux run proves the smoke
...
- `.github/workflows/linux-real-skia-smoke.yml` is a manual Linux job that
  ...
  `skia_mbt native smoke test passed` through
