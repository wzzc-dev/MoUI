...
2. Async image diagnostics
   ...
   - Done when: both host source tests and native provider/platform evidence are
     present: renderer completion/apply ports must consume native async loader
     ready/failed results, matching hosts must record redraw artifacts for late
     completions, or the missing pieces must be explicitly scoped out of the
     preview handoff.
     Renderer-local diagnostics, provider callback forwarding, host
     baseline-guarded redraw scheduling, and host completion-source routing are
     current, but they do not yet prove that every native renderer has a real
