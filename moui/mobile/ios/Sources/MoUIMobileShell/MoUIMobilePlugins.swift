...
public struct MOUIMobilePlatformViewPlacement {
...
public final class MOUIMobilePlatformViewEventSink {
  ...
  public func send(
    ...
    return MOUIMobileRuntimeBridge.shared.dispatchHostResponseEnvelopeJSON(data) > 0
  }
}

public protocol MOUIMobilePlatformViewFactory: AnyObject {
  func makePlatformView(identifier: String) -> UIView
  func updatePlatformView(
    _ view: UIView,
    placement: MOUIMobilePlatformViewPlacement,
    events: MOUIMobilePlatformViewEventSink
  )
  func disposePlatformView(_ view: UIView)
}

extension MOUIMobilePlatformViewFactory {
  public func disposePlatformView(_ view: UIView) {}
}

public enum MOUIMobileHostChannelStatus: String {
  case ok
  case error
  case unavailable
}

public struct MOUIMobileHostChannelRequest {
...
public struct MOUIMobileHostChannelResponse {
  public let status: MOUIMobileHostChannelStatus
  public let payload: String

  public init(status: MOUIMobileHostChannelStatus, payload: String = "") {
    self.status = status
    self.payload = payload
  }
}

public protocol MOUIMobileHostChannelTask: AnyObject {
  func cancel()
}

public final class MOUIMobileHostChannelCompletion {
  private let lock = NSLock()
  private var finished = false
  private var deliver: ((MOUIMobileHostChannelResponse) -> Void)?

  init(deliver: @escaping (MOUIMobileHostChannelResponse) -> Void) {
    self.deliver = deliver
  }

  @discardableResult
  public func complete(_ response: MOUIMobileHostChannelResponse) -> Bool {
    lock.lock()
  ...
  public func succeed(payload: String = "") -> Bool {
    complete(MOUIMobileHostChannelResponse(status: .ok, payload: payload))
  }

  @discardableResult
  public func fail(payload: String) -> Bool {
    complete(MOUIMobileHostChannelResponse(status: .error, payload: payload))
  }

  @discardableResult
  public func unavailable(payload: String) -> Bool {
    complete(MOUIMobileHostChannelResponse(status: .unavailable, payload: payload))
  }
...
public protocol MOUIMobileHostChannelHandler: AnyObject {
  @discardableResult
  func handle(
    request: MOUIMobileHostChannelRequest,
    completion: MOUIMobileHostChannelCompletion
  ) -> MOUIMobileHostChannelTask?
}
...
public final class MOUIMobilePluginRegistry {
  public static let shared = MOUIMobilePluginRegistry()

  private var platformViewFactories: [String: MOUIMobilePlatformViewFactory] = [:]
  private var hostChannelHandlers: [String: MOUIMobileHostChannelHandler] = [:]
  ...
  public func registerPlatformView(
    kind: String,
    factory: MOUIMobilePlatformViewFactory
  ) {
  ...
  public func registerHostChannel(
    name: String,
    handler: MOUIMobileHostChannelHandler
  ) {
  ...
  func platformViewFactory(kind: String) -> MOUIMobilePlatformViewFactory? {
    platformViewFactories[kind]
  }

  func hostChannelHandler(name: String) -> MOUIMobileHostChannelHandler? {
    hostChannelHandlers[name]
  }
}

final class MOUIMobilePlatformViewOverlay {
  ...
  private struct HostedView {
    let factory: MOUIMobilePlatformViewFactory
    let view: UIView
  ...
  func apply(snapshot: [String: Any], sessionGeneration: Int) {
    ...
    for payload in payloads {
      guard
        let placement = MOUIMobilePlatformViewPlacement(payload: payload),
        let factory = MOUIMobilePluginRegistry.shared.platformViewFactory(kind: placement.kind)
      else { continue }
      ...
      let events = MOUIMobilePlatformViewEventSink(
        sessionGeneration: sessionGeneration,
        revision: nextRevision,
