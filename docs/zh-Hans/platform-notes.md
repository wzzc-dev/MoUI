...
MoUI 从 MoonBit registry 解析修改后的窗口宿主为 `wzzc-dev/window@0.5.4-0.1.3`。仓库本地 window checkout 不再属于普通开发流程。该 fork 包目前提供上游包尚未覆盖、但 MoUI 需要的目标支持。主 checkout 现在包含 `moui_skia`，它提供原生 Skia 光栅主线渲染器使用的可编辑 Skia 绑定。
...
Web 通过同一 registry/slot 路径创建主窗口，`WebAppOptions` 由 `@web.entry` 捕获并支持 resolver-backed `OpenWindow` 请求。原生宿主核心通过同一路径创建平台窗口，但不自行选择具体渲染器族。应用入口点向 `@runtime.run_app` 提供有序 `RendererBindingFactory` 和一个平台 `PlatformEntry`。已解析窗口创建中立 `HostSurfaceKit`，解析出 `HostWindowRenderer`，随后注册 `HostRuntimeDriver`、平台绑定和平台 slot，并按 `HostWindowId` 路由重绘、事件、上下文菜单、宿主服务 completion、IME 同步和释放。如果没有 scene resolver，宿主会用共享 unavailable-resolver 响应拒绝 `OpenWindow`。

原生渲染器选择属于应用组合，不是 host-core options 字段。Skia 主线使用 `render/skia.from_env()`，原生 WGPU 诊断使用 `render/wgpu.native(...)`。Android、iOS 和 HarmonyOS 使用 `wzzc-dev/window` 作为嵌入运行时后端的 embedder。template 通过 `EventLoop` 将 `HostCmd` 交给匹配的 `*EmbeddedRuntimeBackend`，后者装配 MoUI runtime session 和中立宿主表面。lifecycle、surface 和 input 不得绕过该路线。
...
- Sun factory 会显式报告渲染器侧平台视图像素接线。macOS、Windows 和 Linux Sun 在其 `HostWindowRenderer` 包装器把离屏平台视图像素转发进 Sun 帧时报告 `renderer_platform_view_pixels=SunRasterRenderer.draw_platform_view_pixels`。这是应用层 renderer composition 接线；在提出更宽的平台运行时就绪声明前，仍需要匹配宿主运行时 smoke。
...
- 权限或回调驱动的宿主服务可以在 host adapter 内使用 `HostServiceAsyncQueue` 并返回 `HostServiceResponse::Pending`，而不是阻塞运行时。应用侧只接收 `ServiceTask[T]`，并通过 `ServiceTask::effect` 把成功、失败或取消转换为 typed `Msg`；request id、bridge 和 completion queue 不离开 `backend/host`，业务 `Model` 不保存平台请求状态。
