...
export const readMouiPluginManifests = (paths, { workspaceRoot } = {}) => {
  ...
  const ids = new Set();
  for (const plugin of plugins) {
    if (ids.has(plugin.id)) fail("shell.plugins", `duplicate plugin id ${JSON.stringify(plugin.id)}`);
    ids.add(plugin.id);
  }
  assertGloballyUnique(plugins, "platformViewKinds", "shell.plugins");
  assertGloballyUnique(plugins, "hostChannels", "shell.plugins");
  return plugins;
};
