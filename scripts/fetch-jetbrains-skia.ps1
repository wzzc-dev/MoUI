param(
  ...
  [string] $CacheDir = ".skia-cache/jetbrains",
...
function Get-HostPlatform {
  ...
  throw "cannot auto-detect JetBrains Skia platform"
}
...
function Get-HostArch {
  switch ([System.Runtime.InteropServices.RuntimeInformation]::ProcessArchitecture.ToString().ToLowerInvariant()) {
    ...
    "x64" { return "x64" }
    "amd64" { return "x64" }
    default { throw "cannot auto-detect JetBrains Skia architecture: $([System.Runtime.InteropServices.RuntimeInformation]::ProcessArchitecture)" }
  }
}
...
if ($resolvedPlatform -eq "auto") {
  $resolvedPlatform = Get-HostPlatform
}

$resolvedArch = $Arch
if ($resolvedPlatform -eq "wasm") {
  if ($resolvedArch -ne "auto") {
    throw "-Platform wasm only supports -Arch auto with the wasm manifest asset"
  }
  $resolvedArch = "wasm"
...
if (!$provider) {
  throw "skia-provider-lock.json is missing providers.jetbrains"
}
if ($provider.tag -ne $Tag) {
  throw "manifest only locks JetBrains tag $($provider.tag), requested $Tag"
}

$platformAssets = Get-JsonPropertyValue -Object $provider.assets -Name $resolvedPlatform
if (!$platformAssets) {
  throw "manifest has no JetBrains assets for platform=$resolvedPlatform"
}
$configAssets = Get-JsonPropertyValue -Object $platformAssets -Name $Config
if (!$configAssets) {
  throw "manifest has no JetBrains assets for platform=$resolvedPlatform config=$Config"
}
$asset = Get-JsonPropertyValue -Object $configAssets -Name $resolvedArch
if (!$asset) {
  throw "manifest has no JetBrains asset for platform=$resolvedPlatform config=$Config arch=$resolvedArch"
}

$assetName = $asset.name
$assetSha256 = $asset.sha256.ToLowerInvariant()
$assetSize = $asset.size
$assetUrl = if ($asset.PSObject.Properties["url"]) {
  $asset.url
} else {
  "https://github.com/JetBrains/skia/releases/download/$Tag/$assetName"
}
$sourceName = if ($provider.source_archive.name) { $provider.source_archive.name } else { "JetBrains-skia-$Tag-source.zip" }
$sourceUrl = if ($provider.source_archive.url) { $provider.source_archive.url } else { "https://github.com/JetBrains/skia/archive/refs/tags/$Tag.zip" }
...
$entryDir = Join-Path $tagDir "$resolvedPlatform-$Config-$resolvedArch"
...
if (!$skiaInclude) {
  ...
  if (!$DryRunConfig) {
    ...
    if (!$skiaInclude) {
      throw "Skia headers were not found in package or source archive for tag $Tag"
    }
  } else {
...
$skiaLibDir = Find-LibDir -Root $packageDir -ResolvedPlatform $resolvedPlatform
if (!$skiaLibDir) {
  if ($DryRunConfig) {
    $skiaLibDir = $packageDir -replace "\\", "/"
  } else {
    throw "Skia library for platform=$resolvedPlatform was not found in $packageDir"
  }
}
...
if ($PrintEnv) {
  @(
    ...
    "SKIA_MBT_SKIA_ROOT=$skiaRoot"
    "SKIA_MBT_JETBRAINS_TAG=$Tag"
    "SKIA_MBT_SKIA_COMMIT=$($provider.commit)"
    "SKIA_MBT_SKIA_PACKAGE=$assetName"
    "SKIA_MBT_SKIA_PACKAGE_SHA256=$assetSha256"
    "SKIA_MBT_SKIA_INCLUDE=$skiaInclude"
    "SKIA_MBT_SKIA_LIB_DIR=$skiaLibDir"
    "SKIA_MBT_SKIA_LIB=skia"
    "SKIA_MBT_EXTRA_CC_FLAGS=$extraCcFlags"
    "SKIA_MBT_EXTRA_LINK_FLAGS=$extraLinkFlags"
  ) | Write-Output
  exit 0
}
...
if ($DryRunConfig) {
  Write-Output "Dry run complete; JetBrains Skia was not downloaded or extracted."
}
