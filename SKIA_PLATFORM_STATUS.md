...
- The native smoke log passed `scripts/verify-native-smoke-log.*`, including
  ...
  fallback resource planning, canvas paint-shader fill replay,
  linear-gradient shader validation, invalid shader/filter descriptor cache
  safety, invalid shader/filter replay skip and zero-cache-touch proof,
  non-finite filter validation, and shader/filter resource planning.
