...
static const NSInteger MOUI_IOS_HOST_IMAGE_VIEW_TAG = 0x4D4F5549;

static int64_t moui_ios_host_expected_pixel_bytes(int32_t width, int32_t height) {
...
static UIImageView *moui_ios_host_image_view(UIView *view, CGRect frame) {
  UIImageView *image_view = nil;
  for (UIView *subview in view.subviews) {
    if (subview.tag == MOUI_IOS_HOST_IMAGE_VIEW_TAG) {
  ...
  if (image_view == nil) {
    ...
    image_view.tag = MOUI_IOS_HOST_IMAGE_VIEW_TAG;
...
static int32_t moui_ios_present_pixels_to_view_main(
  ...
  UIImageView *image_view = moui_ios_host_image_view(view, bounds);
  if (image_view == nil) {
...
int32_t moui_ios_present_pixels_to_view(uint64_t raw_view_handle,
  ...
  int64_t expected_len = moui_ios_host_expected_pixel_bytes(width, height);
  ...
  if ([NSThread isMainThread]) {
    return moui_ios_present_pixels_to_view_main(raw_view_handle, width,
                                                     height, data);
  }
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    (void)moui_ios_present_pixels_to_view_main(raw_view_handle, width,
                                                    height, data);
  });
  return 1;
}
...
/// non-zero handle. The caller retains the layer; host borrows per-frame
/// drawables from it without taking ownership.
extern "C" MOONBIT_FFI_EXPORT
uint64_t moui_ios_metal_layer_for_view(uint64_t raw_view_handle) {
  if (raw_view_handle == 0) {
    return 0;
  }
