// Linux async image I/O: background file reading via pthread, results drained
// on the main thread. The background thread only uses C stdlib (fopen/fread)
// and malloc — it never touches MoonBit GC objects. The main-thread drain
// copies malloc'd buffers into moonbit_bytes_t and frees the originals.
...
static void *moui_async_image_read_file(void *arg) {
...
MOONBIT_FFI_EXPORT
void moui_linux_async_image_spawn(int64_t window_id,
...
MOONBIT_FFI_EXPORT
moonbit_bytes_t moui_linux_async_image_take_result(void) {
  pthread_mutex_lock(&g_mutex);
