...
2. **Component token** - `@core.ComponentThemes.button : ButtonTheme`
   （`moui/core/theme_components.mbt`、`moui/core/theme_resolver.mbt`）。
   Ownership note（S1）：这些 token struct 保留在 `@core.Theme.components` 上，这样 resolver 和 `moui_theme` 可以在没有 `core → views` 的情况下投影。面向 app 的一次性 style 仍是 `@views.ButtonStyle` / variant helper。参见 `docs/plans/active/core-component-theme-to-views.md`。
   - `minimal_components()` 从 palette 构建默认 `ButtonTheme`。每个 variant（`primary`/`tonal`/`outline`/`ghost`/`subtle`/`subtle_brand`）都是一个 `ControlStateTokens`（foreground/background/border/border_width/radius）。
   ...
   - 品牌 addon（`moui_theme/*`）会通过 `DesignComponentTokens::core_themes(scheme)` 整体替换 `components`；Fluent 2 还会区分 `subtle`/`subtle_brand` 并添加 brand-stroke border。
...
4. **控件绘制** - `button_control`（`moui/views/control_primitives.mbt`），由 `button`（`moui/views/button.mbt`）调用。
   - `resolved_style = style? | variant.style(resolved_theme)` - 显式 `style=` 参数是一次性覆盖；否则 variant 从 `theme.components.button` 解析（第 2/3 层）。
   - `variant.style(theme)`（`moui/views/style_api.mbt`）将 `ButtonVariant` → `ButtonVariantToken` → `ButtonTheme::resolve`。
   - Theme 在 paint time 通过 ambient 解析：`theme.unwrap_or(ctx.environment.theme)`，因此 dark-mode/a11y/reduced-motion 变化无需 caller 重建 view tree 即可生效。
...
`ControlStateStyle` 在 `@core` 中是 `pub(all)`，并通过 `@views` 重导出。App 包通过 `@views.new_control_state_style(...)` 构造它。`ButtonStyle` 是 `pub struct`（字段私有）；通过 `ButtonStyle::new(...)` 或 factory method（`::filled`、`::tonal` 等）构造，并通过 `with_xxx(...)` method 修改（Flutter `copyWith` 模式）。

### 策略 B - Theme 层 component 覆盖（对一个 app 全局生效）

当 app 中每个 Primary button 都应改变时使用。在构建 theme 的位置一次性覆盖 `theme.components.button.primary`，并把 patch 后的 theme 传给每个控件。这样保留 state-layer hover/press 模型不变：只改变 normal token，hover/press 仍会派生出来。
...
`ComponentThemes`、`ButtonTheme`、`ControlStateTokens` 和 `StateLayerTokens` 在 `@core` 中都是 `pub(all)`，因此 app 包可以读取 `base.components` 并用 struct-spread（`{ ..base.components, button }`）重建。
...
- **为了改变一个 app 的 button 颜色而编辑 `moui/core/theme.mbt` 或 `moui/views/control_style.mbt` 几乎总是错的。** 这些是框架文件，会影响每个 app。改用 app 包中的策略 A 或 B。
- **`ButtonStyle::filled/tonal/outline/ghost` 是 legacy helper**，会绕过 `theme.components.button`。它们仍可作为 `style=` 基础使用，但不会跟随品牌 component token。为保持与品牌设计系统一致，优先 patch `theme.components.button`（策略 B）。
