...
Options:
  ...
  --manifest <path>       Manifest path. Default artifacts/shell-runtime/<platform>/<app>/shell-runtime-smoke.json.
...
const observeLogs = (observations, logs, scrollRequired, pixelChange) => {
  if (
    logs.includes("moui-shell lifecycle attach")
    || /moui-shell attach app=/.test(logs)
  ) {
    observations.lifecycleAttach = "yes";
  }
  if (hasMobileResizeTransition(logs)) observations.resize = "yes";
  const receivedPointer = logs.includes("moui-shell input pointer");
  const receivedScroll = logs.includes("moui-shell input scroll");
  ...
  if (
    logs.includes("moui-shell service ime edit")
    || (logs.includes("moui-shell service ime state")
      && /ime state enabled=(true|1)/.test(logs)
      && receivedPointer)
  ) {
    if (logs.includes("moui-shell service ime edit")) observations.ime = "yes";
  }
  if (hasMobileTextClipboardRoundTrip(logs)) observations.clipboard = "yes";
  if (logs.includes("moui-shell service accessibility tree")) observations.accessibilityTree = "yes";
  if (logs.includes("moui-shell service accessibility focus")) observations.accessibilityFocus = "yes";
  if (logs.includes("moui-shell service accessibility action")) observations.accessibilityAction = "yes";
  ...
  if (logs.includes("moui-shell service async-image phase=loading")
      && logs.includes("moui-shell service async-image phase=ready")) {
...
// Stream `adb logcat` continuously (filtered to the MoUIShell tag) so
...
const startAndroidLogStream = ({ device, outPath }) => {
  ...
  const child = spawn(
    "adb",
    [...serialArgs, "logcat", "-s", "MoUIShell:V"],
    {
      cwd: repoRoot,
      stdio: ["ignore", fd, fd],
    },
...
const runAndroidSmoke = ({ appConfig, artifact, logPath, beforePath, screenshotPath, device, assistiveTech, scrollRequired }) => {
  ...
  const launchArgs = [
    ...
    `${appConfig.android.applicationId}/dev.wzzc.moui.shell.MoUIActivity`,
  ];
  if (appConfig.id === "showcase") {
    launchArgs.push("--es", "moui.shell.testProbe", "1");
  }
  ...
  if (probePlan) {
    ...
    result = run("adb", [...serialArgs, "shell", "input", "text", "ime-shell-probe"]);
  ...
  observations.lifecycleDetach = (
    logs.includes("moui-shell lifecycle detach")
    || logs.includes("moui-shell detach")
  ) ? "yes" : "no";
...
const startIosLogStream = ({ target, productName, outPath }) => {
  ...
  const child = spawn(
    ...
    [
      ...
      "--style", "compact",
      "--predicate",
      `process == '${productName}' AND eventMessage CONTAINS 'moui-shell'`,
    ],
    {
      cwd: repoRoot,
      stdio: ["ignore", fd, fd],
...
const runIosSmoke = ({ appConfig, artifact, logPath, beforePath, screenshotPath, device, assistiveTech, scrollRequired }) => {
  ...
  if (appConfig.id === "showcase") {
    launchEnv.SIMCTL_CHILD_MOUI_EMBEDDING_TEST_PROBE = "1";
    launchEnv.MOUI_EMBEDDING_TEST_PROBE = "1";
  }
  ...
  if (serviceProbePlan) {
    ...
    action = run("idb", [
      "ui", "text", "ime-shell-probe", "--udid", idbTarget,
    ]);
  ...
  if (!streamLogs.includes("moui-shell lifecycle attach")) {
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 500);
    streamLogs = existsSync(streamPath) ? readFileSync(streamPath, "utf8") : streamLogs;
  }
  const appLogPredicate = launchPid
    ? `processIdentifier == ${launchPid} AND eventMessage CONTAINS 'moui-shell'`
    : `process == '${appConfig.ios.productName}' AND eventMessage CONTAINS 'moui-shell'`;
  result = run("xcrun", [
  ...
  appendLog(logPath, "ios merged log markers", {
    ...
    stdout: [
      ...
      `hasAttach=${logs.includes("moui-shell lifecycle attach")}`,
      `hasImeState=${logs.includes("moui-shell service ime state")}`,
      `hasImeEdit=${logs.includes("moui-shell service ime edit")}`,
      `hasClipboardWrite=${logs.includes("moui-shell service clipboard complete operation=write-text")}`,
      `hasClipboardRead=${logs.includes("moui-shell service clipboard complete operation=read-text")}`,
      `hasAsyncLoading=${logs.includes("moui-shell service async-image phase=loading")}`,
      `hasAsyncReady=${logs.includes("moui-shell service async-image phase=ready")}`,
      `hasA11yFocus=${logs.includes("moui-shell service accessibility focus")}`,
      `hasA11yAction=${logs.includes("moui-shell service accessibility action")}`,
    ].join("\n"),
    stderr: "",
  });
  const pixelChange = compareScreenshots(beforePath, screenshotPath);
  ...
  observations.lifecycleDetach = hasIosApplicationLog(
    ...
    "moui-shell lifecycle detach",
  ) || detachLogs.includes("moui-shell lifecycle detach")
    ? "yes"
    : "no";
...
const runHarmonySmoke = ({ appConfig, artifact, logPath, beforePath, screenshotPath, device, scrollRequired }) => {
  ...
  if (appConfig.id === "showcase") {
    launchArgs.push("--ps", "moui.shell.testProbe", "1");
  }
  ...
  if (probePlan) {
    ...
    Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 400);
    result = run("hdc", [...targetArgs, "shell", "uitest", "uiInput", "inputText", "ime-shell-probe"]);
...
try {
  ...
  if (!appConfig) throw new Error(`unknown shell app: ${options.app}`);
  ...
  const outDir = options.manifest
    ...
    : join(repoRoot, "artifacts/shell-runtime", options.platform, options.app);
  ensureDir(outDir);
  const manifestPath = options.manifest
    ? normalize(options.manifest)
    : join(outDir, "shell-runtime-smoke.json");
  ...
  let renderer = parseShellRendererStatus(runtimeLogs);
  if (!renderer) {
    const buildJsonPath = join(dirname(artifact), "shell-build.json");
  ...
  const manifest = {
    schemaVersion: 1,
    mode: "shell-runtime-smoke",
    generatedBy: "scripts/record-shell-runtime-smoke.mjs",
    ...
    build: {
      ...
      command: options.platform === "android"
        ? `scripts/build-shell-android-apk.sh --app ${options.app}`
        : options.platform === "ios"
          ? `scripts/build-shell-ios-app.sh --app ${options.app}`
          : `scripts/build-shell-harmonyos-hap.sh --app ${options.app}`,
    },
    artifacts: {
  ...
  const validatorArgs = ["scripts/validate-shell-runtime-manifest.mjs", manifestPath];
  ...
  console.error(`[moui-shell-smoke] ${error.message}`);
