...
final class MOUIMobileTextProxy: UITextView {
  let runtimeBridge = MOUIMobileRuntimeBridge.shared
...
final class MOUIMobileIMEAdapter: NSObject, UITextViewDelegate {
  private let bridge = MOUIMobileRuntimeBridge.shared
  private let proxy = MOUIMobileTextProxy(frame: .zero)
...
final class MOUIMobileAccessibilityElement: UIAccessibilityElement {
  var elementIdentifier: Int32 = 0
  var actions: Set<String> = []
  let bridge = MOUIMobileRuntimeBridge.shared
...
final class MOUIMobileAccessibilityAdapter {
  ...
  func apply(_ payload: [String: Any]) {
    ...
    container.accessibilityElements = nodes.map { node in
      let element = MOUIMobileAccessibilityElement(accessibilityContainer: container)
...
final class MOUIMobileHostAdapter {
  ...
  private final class PendingHostChannel {
    let completion: MOUIMobileHostChannelCompletion
    var task: MOUIMobileHostChannelTask?

    init(completion: MOUIMobileHostChannelCompletion) {
  ...
  private func applyPlatformChannel(
    ...
    let completion = MOUIMobileHostChannelCompletion { [weak self] response in
    ...
    guard let request = MOUIMobileHostChannelRequest(payload: payload) else {
      completion.fail(payload: "invalid platform channel request")
      return
    }
    guard
      let handler = MOUIMobilePluginRegistry.shared.hostChannelHandler(
  ...
  private func finishPlatformChannel(
    ...
    response: MOUIMobileHostChannelResponse
  ) {
