...
import { defaultMouiRoot, defaultSkiaRoot, defaultWorkspaceRoot, readShellApp } from "./app-config.mjs";
...
import { prepareAndroidPlugins } from "../android/embedder/prepare-plugins.mjs";
import { resolveAndroidManagedShell } from "../android/runner/resolve-shell.mjs";

const usage = `Usage: moui_shell/scripts/prepare-native-build.mjs --platform android|ios|harmonyos --app <id> --build-dir <dir> [options]

Options:
  --platform <name>          android, ios, or harmonyos.
  --app <id>                 Registered shell app id.
  ...
  --android-shell <mode>     Android managed or ejected shell, default managed.
...
const copyAndroidSharedLibs = ({ abi, ndkHome, skiaLinkFlags, jniLibsDir }) => {
  ...
  if (libcxx && existsSync(libcxx)) {
    ...
    console.log(`[moui-shell-android] packaged libc++_shared.so from ${libcxx} (${size} bytes)`);
  } else {
    console.warn(
      `[moui-shell-android] libc++_shared.so not found for abi=${abi} ndkHome=${ndkHome}; ` +
...
const generateMoonbitC = ({ workspaceRoot, moonPackage, generatedC, buildDir }) => {
  ...
  if (result.status !== 0) {
    console.error(`[moui-shell-prepare] moon build returned ${result.status}; continuing because generated C exists: ${moonbitC}`);
  }
};

const writeAndroidCmakeConfig = ({ config, app, buildDir, moonbitC, skia, abi, rendererSelection, workspaceRoot, mouiRoot, skiaRoot, shellRoot }) => {
  const nativeDir = join(buildDir, "native");
  ensureDir(nativeDir);
  const cmakePath = join(nativeDir, "moui-shell-native.cmake");
  const lines = [
    "# Generated by moui_shell/scripts/prepare-native-build.mjs.",
    `set(MOUI_WORKSPACE_ROOT "${quoteCmake(workspaceRoot)}")`,
    `set(MOUI_ROOT "${quoteCmake(mouiRoot)}")`,
    `set(MOUI_SHELL_ROOT "${quoteCmake(shellRoot)}")`,
    `set(MOUI_SKIA_ROOT "${quoteCmake(skiaRoot)}")`,
    `set(MOUI_MOON_HOME "${quoteCmake(process.env.MOON_HOME || join(process.env.HOME || "", ".moon"))}")`,
    `set(MOUI_EMBEDDING_LIBRARY_NAME "${quoteCmake(config.nativeLibrary)}")`,
    `set(MOUI_EMBEDDING_MOONBIT_C "${quoteCmake(moonbitC)}")`,
    `set(MOUI_EMBEDDING_APP_ARG "${quoteCmake(config.appArg)}")`,
    `set(MOUI_EMBEDDING_APP_ID "${quoteCmake(app)}")`,
    `set(MOUI_EMBEDDING_RENDERER_REQUESTED "${quoteCmake(rendererSelection.requested)}")`,
    `set(MOUI_EMBEDDING_RENDERER_SELECTED "${quoteCmake(rendererSelection.selected)}")`,
    `set(MOUI_SKIA_STUB_CC_FLAGS "${quoteCmake(skia.stubFlags)}")`,
    `set(MOUI_SKIA_CC_LINK_FLAGS "${quoteCmake(skia.linkFlags)}")`,
    `set(MOUI_EMBEDDING_ANDROID_ABI "${quoteCmake(abi)}")`,
    "",
  ];
...
const writeHarmonyosCmakeConfig = ({ config, app, buildDir, moonbitC, skia, ohosArch, rendererSelection, workspaceRoot, mouiRoot, skiaRoot, shellRoot }) => {
  const nativeDir = join(buildDir, "native");
  ensureDir(nativeDir);
  const cmakePath = join(nativeDir, "moui-shell-harmonyos.cmake");
  const lines = [
    "# Generated by moui_shell/scripts/prepare-native-build.mjs.",
    `set(MOUI_WORKSPACE_ROOT "${quoteCmake(workspaceRoot)}")`,
    `set(MOUI_ROOT "${quoteCmake(mouiRoot)}")`,
    ...
    `set(MOUI_EMBEDDING_LIBRARY_NAME "${quoteCmake(config.nativeLibrary)}")`,
    `set(MOUI_EMBEDDING_MOONBIT_C "${quoteCmake(moonbitC)}")`,
    `set(MOUI_EMBEDDING_MOONBIT_MAIN_ALIAS "${quoteCmake(config.moonbitMainAlias)}")`,
    `set(MOUI_EMBEDDING_APP_ARG "${quoteCmake(config.appArg)}")`,
    `set(MOUI_EMBEDDING_APP_ID "${quoteCmake(app)}")`,
    `set(MOUI_EMBEDDING_RENDERER_REQUESTED "${quoteCmake(rendererSelection.requested)}")`,
    `set(MOUI_EMBEDDING_RENDERER_SELECTED "${quoteCmake(rendererSelection.selected)}")`,
    `set(MOUI_EMBEDDING_ENABLE_SCROLL ${config.supportsScroll ? "ON" : "OFF"})`,
    `set(MOUI_EMBEDDING_ATTACH_SURFACE_SYMBOL "${quoteCmake(config.exports.attachSurface)}")`,
    `set(MOUI_EMBEDDING_RESIZE_SYMBOL "${quoteCmake(config.exports.resize)}")`,
    `set(MOUI_EMBEDDING_DISPATCH_POINTER_SYMBOL "${quoteCmake(config.exports.dispatchPointer)}")`,
    `set(MOUI_EMBEDDING_DISPATCH_SCROLL_SYMBOL "${quoteCmake(config.exports.dispatchScroll || "moui_embedding_no_scroll")}")`,
    `set(MOUI_EMBEDDING_FRAME_TICK_SYMBOL "${quoteCmake(config.exports.frameTick)}")`,
    `set(MOUI_EMBEDDING_RENDER_FRAME_SYMBOL "${quoteCmake(config.exports.renderFrame)}")`,
    `set(MOUI_EMBEDDING_DETACH_SURFACE_SYMBOL "${quoteCmake(config.exports.detachSurface)}")`,
    `set(MOUI_SKIA_STUB_CC_FLAGS "${quoteCmake(skia.stubFlags)}")`,
    `set(MOUI_SKIA_CC_LINK_FLAGS "${quoteCmake(skia.linkFlags)}")`,
    `set(MOUI_EMBEDDING_HARMONYOS_OHOS_ARCH "${quoteCmake(ohosArch)}")`,
    "",
  ];
  writeFileSync(cmakePath, lines.join("\n"));
  return cmakePath;
...
const prepareAndroid = ({ app, appConfig, config, plugins, buildDir, abi, runnerMode, renderer, fallbackSkia, workspaceRoot, mouiRoot, skiaRoot, shellRoot }) => {
  if (!androidAbiToSkiaArch.has(abi)) throw new Error(`unsupported Android ABI: ${abi}`);
  if (appConfig.android.runnerMode !== runnerMode) {
    throw new Error(
      `app ${app} requests Android runnerMode=${appConfig.android.runnerMode}; ${runnerMode} shell required`,
    );
  }
  const androidShellConfig = resolveAndroidManagedShell({ app: appConfig, buildDir, workspaceRoot });
  ...
  const cmakeConfig = writeAndroidCmakeConfig({ config, app, buildDir, moonbitC, skia, abi, rendererSelection, workspaceRoot, mouiRoot, skiaRoot, shellRoot });
  writeBuildJson(join(buildDir, "shell-build.json"), {
    schemaVersion: 1,
    platform: "android",
    app,
    moonbitC,
...
const prepareIos = ({ app, config, buildDir, sdk, arch, renderer, fallbackSkia, workspaceRoot, mouiRoot, skiaRoot, shellRoot }) => {
...
const prepareHarmonyos = ({ app, config, buildDir, arch, renderer, fallbackSkia, workspaceRoot, mouiRoot, skiaRoot, shellRoot }) => {
  ...
  validateHarmonyosSkiaLinkCompatibility({
    renderer: rendererSelection.selected,
    linkFlags: skia.linkFlags,
  });
  const ohosArch = harmonyosArchToOhosArch.get(arch);
  const cmakeConfig = writeHarmonyosCmakeConfig({ config, app, buildDir, moonbitC, skia, ohosArch, rendererSelection, workspaceRoot, mouiRoot, skiaRoot, shellRoot });
  const sharedLibs = [];
...
try {
  ...
  if (options.platform === "android" && !["managed", "ejected"].includes(options.androidShell)) {
    throw new Error("--android-shell must be managed or ejected");
  }
  ...
  const appConfig = readShellApp(options.app, {
    workspaceRoot,
    mouiRoot,
    skiaRoot,
    appConfigPath: options.appConfigPath,
  });
  const shellRoot = appConfig.paths.shellRoot;
  ...
  if (options.platform === "android") {
    prepareAndroid({
      ...
      buildDir,
      abi: options.abi,
      runnerMode: options.androidShell,
  ...
  console.error(`[moui-shell-prepare] ${error.message}`);
